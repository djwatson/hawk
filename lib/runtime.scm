...
(define (integer->char x)
  (unless (and (fixnum? x) (<= 0 x #x10ffff)
               (or (< x #xd800) (> x #xdfff)))
    (error "bad integer->char"))
  (sys:INTEGER_CHAR x))
...
(define (list-tail lst k)
  (unless (and (fixnum? k) (not (negative? k)))
    (error "list-tail: expected nonnegative exact integer" k))
...
(define (list-ref lst n)
  (unless (and (fixnum? n) (not (negative? n)))
    (error "list-ref: expected nonnegative exact integer" n))
          ...
          (let ((hare (cdr hare)))
            (if (pair? hare)
                (and (not (eq? hare tortoise))
                     (loop (cdr hare)
                           (if (pair? tortoise) (cdr tortoise) hare)))
                #t))
          #t)))
  (cond
    ((not (pair? lst)) lst)
    ((null? lst) lst)
    ((and (pair? lst) (finite-pairs? lst))
     (let copy ((x lst))
       (if (pair? x) (cons (car x) (copy (cdr x))) x)))
    (else (error "list-copy: not a finite list"))))
...
(define (make-binary-input-port fd)
  (make-port fd #t port-binary-input #f (make-bytevector port-buffer-size) 0 0 #f))
(define (make-binary-output-port fd)
  (make-port fd #t port-binary-output #f (make-bytevector port-buffer-size) 0 0 #f))
...
(define (c-write fd data len)
  (sys:FOREIGN_CALL '(int64 "scm_write_buffer" (int32 gc_obj uint64)) fd data len))

(define (c-read fd buf cnt)
  (sys:FOREIGN_CALL '(int64 "scm_read_buffer" (int32 gc_obj uint64)) fd buf cnt))
...
(define (port-write-all fd data len)
  (let loop ((off 0))
    (if (< off len)
        (let* ((buf (if (= off 0) data
                        (if (string? data) (substring data off len)
                            (bytevector-copy data off len))))
...
(define (read-from-port-buffer port)
  (if (not (textual-input-port? port))
      ...
      (let ((pos (sys:LOAD port 3)) (len (sys:LOAD port 4)))
        (if (sys:LT pos len)
            (let ((c (sys:LOAD_CHAR (sys:LOAD port 6) pos)))
              (sys:STORE port (sys:ADD pos 1) 3)
              c)
            (cond
  ...
  (case-lambda
    ...
    ((port)
      (if (not (textual-input-port? port))
          ...
          (let ((pos (sys:LOAD port 3)) (len (sys:LOAD port 4)))
            (if (sys:LT pos len)
                (sys:LOAD_CHAR (sys:LOAD port 6) pos)
                (cond
                  ((not (sys:LOAD port 1)) (error "Port not open"))
                  ((sys:LT len 0) (make-eof-object))
                  ((sys:LT (sys:LOAD port 0) 0) (sys:STORE port -1 4) (make-eof-object))
...
(define (utf8->string-internal v start end)
  (let ((res (make-string (- end start))))
    (let loop ((i start) (j 0) (v v) (res res) (end end))
      (if (= i end)
          (if (= j (- end start)) res (substring res 0 j))
          (let ((b (bytevector-u8-ref v i)))
            (if (< b 128)
                (begin
                  (string-set! res j (integer->char b))
                  (loop (+ i 1) (+ j 1) v res end))
                (let ((n (cond ((<= 194 b 223) 2)
                               ((<= 224 b 239) 3) ((<= 240 b 244) 4)
                               (else (error "Invalid UTF-8")))))
                  (when (> (+ i n) end) (error "Incomplete UTF-8"))
                  (let decode ((k 1) (c (modulo b (case n ((2) 32) ((3) 16) (else 8)))))
                    (if (= k n)
                        (begin
                          (when (< c (case n ((2) 128) ((3) 2048) (else 65536)))
                            (error "Overlong UTF-8"))
                          (string-set! res j (integer->char c))
                          (loop (+ i n) (+ j 1) v res end))
                        (let ((b (bytevector-u8-ref v (+ i k))))
                          (unless (<= 128 b 191) (error "Invalid UTF-8"))
                          (decode (+ k 1) (+ (* c 64) (- b 128)))))))))))))
(define utf8->string
  (case-lambda
    ((v)
...
(define (string->utf8-internal v start end)
  (let* ((len (let loop ((i start) (len 0))
                (if (= i end)
                    len
                    (let ((c (char->integer (string-ref v i))))
                      (loop (+ i 1)
                            (+ len (cond ((< c 128) 1) ((< c 2048) 2)
                                         ((< c 65536) 3) (else 4))))))))
    ...
    (let loop ((i start) (j 0))
      (if (= i end)
          res
          (let* ((c (char->integer (string-ref v i)))
                 (n (cond ((< c 128) 1) ((< c 2048) 2)
                          ((< c 65536) 3) (else 4))))
            (if (= n 1)
                (bytevector-u8-set! res j c)
                (begin
                  (let encode ((k (- n 1)) (c c))
                    (if (= k 0)
                        (bytevector-u8-set! res j
                          (+ (case n ((2) 192) ((3) 224) (else 240)) c))
                        (begin
                          (bytevector-u8-set! res (+ j k) (+ 128 (modulo c 64)))
                          (encode (- k 1) (quotient c 64)))))))
            (loop (+ i 1) (+ j n)))))))
(define string->utf8
  (case-lambda
    ((v)
