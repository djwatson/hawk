    ...
    (let loop ((i start) (j 0))
      (if (= i end)
          ...
          (let* ((b (bytevector-u8-ref v i))
                 (n (cond ((< b 128) 1) ((<= 194 b 223) 2)
                          ((<= 224 b 239) 3) ((<= 240 b 244) 4)
                          (else (error "Invalid UTF-8")))))
            (when (> (+ i n) end) (error "Incomplete UTF-8"))
            (let decode ((k 1) (c (modulo b (case n ((1) 128) ((2) 32) ((3) 16) (else 8)))))
              (if (= k n)
                  (let ((c (if (= n 1) b c)))
                    (when (< c (case n ((1) 0) ((2) 128) ((3) 2048) (else 65536)))
...
(define (string->utf8-internal v start end)
  (let ((res (make-bytevector (* 4 (- end start)))))
    (let loop ((i start) (j 0))
      (if (= i end)
          (bytevector-copy res 0 j)
