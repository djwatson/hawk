...
(define (obj-tag c)
  (cond
    ...
    ((vector? c) vector-tag)
    ((fun? c) ptr-tag)
    ((const-closure? c) closure-tag)
    (else (error "No object tag:" c))))

(define (obj-size c)
  (align8
    (cond
      ((flonum? c) 16)
      ((string? c) (+ 17 (string-length c)))
      ((symbol? c) 40)
      ((pair? c) 24)
      ((vector? c) (+ 16 (* 8 (vector-length c))))
      ((const-closure? c) 24)
      ((fun? c)
        (+ 32
           (* 8 (hash-table-size (fun-consts c)))
           (* 4 (length (reverse (fun-code c))))))
      (else (error "Can't size object:" c)))))
...
(define (emit-image objects canon)
  ...
  (define (w16 v) (write-u16 v p) (set! pos (+ pos 2)))
  (define (w32 v) (write-u32 v p) (set! pos (+ pos 4)))
  (define (w64 v) (write-u64 v p) (set! pos (+ pos 8)))
  (define (wdouble v) (write-double v p) (set! pos (+ pos 8)))
  (define (pad-to n) (when (< pos n) (w8 0) (pad-to n)))
  (define (canon-of c) (hash-table-ref canon c))
  (define (word c)
    (cond
      ((heap-obj? c) (+ (hash-table-ref offs (canon-of c)) (obj-tag c)))
      ((fixnum? c) (* 8 c))
      ((char? c) (+ char-tag (* 256 (char->integer c))))
      ((boolean? c) (if c true-rep false-rep))
      ((null? c) nil-tag)
      (else (error "Can't encode value:" c))))
  (for-each (lambda (o)
              (hash-table-set! offs o cur)
              (set! cur (+ cur (obj-size o))))
            objects)
  (for-each
    (lambda (o)
      (define start (hash-table-ref offs o))
      (define end (+ start (obj-size o)))
      (when (not (= pos start)) (error "Offset mismatch:" pos start))
      (cond
        ((flonum? o)
          (w32 flonum-tag) (w32 0) (wdouble o))
        ((string? o)
          (w32 string-tag) (w32 0) (w64 (* 8 (string-length o)))
          (string-for-each (lambda (c) (w8 (char->integer c))) o)
          (w8 0))
        ((symbol? o)
          (w32 symbol-tag) (w32 0)
          (w64 (word (symbol->string o)))
          (w64 dead-tag)
          (w64 0)
          (w64 0))
        ((pair? o)
          (w32 cons-tag) (w32 0) (w64 (word (car o))) (w64 (word (cdr o))))
        ((vector? o)
          (w32 vector-tag) (w32 0) (w64 (* 8 (vector-length o)))
          (vector-for-each (lambda (v) (w64 (word v))) o))
        ((const-closure? o)
          (w32 closure-tag) (w32 0) (w64 8)
          (w64 (word (const-closure-fun o))))
        ((fun? o)
          (let* ((code (reverse (fun-code o)))
                 (consts (reverse (fun-consts-list o)))
                 (const-cnt (hash-table-size (fun-consts o))))
            (w32 func-tag) (w32 0) (w64 (word (fun-name o)))
            (w64 const-cnt) (w64 (length code))
            (for-each (lambda (c) (w64 (word c))) consts)
            (for-each
              (lambda (ins idx)
                (let ((op (first ins)))
                  (unless (assq op opcodes) (error "Unknown opcode:" op))
                  (w8 (cdr (assq op opcodes)))
                  (w8 (second ins))
                  (cond
                    ((memq op '(LOOKUP CONST DEFINE))
                      (let* ((const-off (- const-cnt (third ins)))
                             (code-off (+ idx (* 2 const-off))))
                        (unless (fits-in-int16 code-off) (error "Bad const offset"))
                        (w16 code-off)))
                    ((memq op ops_abc) (w8 (third ins)) (w8 (fourth ins)))
                    ((memq op ops_ad) (w16 (third ins)))
                    ((memq op ops_a) (w16 0))
                    (else (error "Unknown op type:" ins)))))
              code
              (iota (length code)))))
        (else (error "Can't emit object:" o)))
      (pad-to end))
    objects)
...
(define (compile-file file)
  ...
  (parameterize ((funs (make-funs-list)))
    (define out (open-binary-output-file (string-append file ".bc")))
    ...
    (let* ((all-funs (get-funs))
      ...
      (define-values (objects canon) (collect-objects roots))
      (define-values (image offs) (emit-image objects canon))
      (string-for-each (lambda (c) (write-u8 (char->integer c) out)) "HAWK")
      (write-u64 0 out)
      (write-u64 (bytevector-length image) out)
      (write-u64 (+ (hash-table-ref offs (hash-table-ref canon main)) ptr-tag) out)
