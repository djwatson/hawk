...
(define (inline-once-called ir)
  ...
  (define (copy-ir ir)
    (cond
      ((ir-application? ir)
        (let ((res (build-application (copy-ir (ir-application-fun ir))
      ...
      ((ir-reference? ir)
        (if (ir-reference-global? ir)
            (build-global-reference (ir-reference-var ir) (ir-reference-mutable? ir) (ir-reference-ann ir))
            (build-lexical-reference (ir-reference-var ir) (ir-reference-mutable? ir) (ir-reference-ann ir))))
      ((ir-assignment? ir)
      ...
      ((ir-begin? ir) (build-begin (map copy-ir (ir-begin-exps ir)) (ir-begin-ann ir)))
      ((ir-void? ir) (build-void))
      ((ir-define? ir) (build-define (ir-define-var ir) (copy-ir (ir-define-exp ir)) (ir-define-ann ir)))
      ((ir-primcall? ir) (build-primcall (ir-primcall-name ir) (map copy-ir (ir-primcall-args ir)) (ir-primcall-ann ir)))
      ...
      ((ir-application? ir)
        (let ((fun (ir-application-fun ir)) (args (ir-application-args ir)))
          (if (and (ir-reference? fun) (hash-table-exists? defs (ir-reference-var fun)))
              (let* ((var (ir-reference-var fun))
                     (init (hash-table-ref defs var)))
                (hash-inc! calls var)
          ...
          ir))
      ((ir-define? ir)
        (collect-uses (ir-define-exp ir) defs calls refs bad)
        ir)
  ...
  (define (rewrite-let ir defs active)
    (let* ((bindings (ir-let-bindings ir))
      ...
      (for-each (lambda (var init)
                  (let ((inline-def (and (not (variable-assigned var))
                                         (let-inline-def init defs))))
      ...
      (let* ((body (rewrite (ir-let-body ir) local-defs active))
             (new-bindings
               (filter-map (lambda (var init)
                             (if (hash-table-exists? local-defs var)
                                 #f
                                 `(,var ,init)))
                           vars
                           inits)))
      ...
      ((ir-begin? ir)
        (let* ((defs (make-hash-table eq?))
          ...
          (for-each (lambda (exp)
                    ...
                    (ir-begin-exps ir))
          (for-each (lambda (exp) (collect-uses exp defs calls refs bad)) (ir-begin-exps ir))
          (for-each (lambda (var)
