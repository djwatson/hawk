                ...
                (lambda ()
                                    ...
                                    (er-macro-transformer (lambda (form rename compare)
                                                            ...
                                                            (let ((condition (cadr form)))
                                                              (if (and (pair? condition)
                                                                       (compare (car condition)
                                                                                (rename 'library)))
                                                                  (if (library-exists? (unwrap-syntax (cadr condition)))
                                                                      (list-ref form 2)
                                                                      (list-ref form 3))
                                                                  (if (memq (unwrap-syntax condition)
                                                                            feature-list)
                                                                      (list-ref form 2)
                                                                      (list-ref form 3)))))))
                  (install-builtin! 'begin-include
                                    (lambda (form env)
                                      ...
                                      (with-current-directory-from-file (cadr form)
                                                                        (lambda (resolved)
                                                                          (build-begin (map (lambda (subform)
                                                                                              (expand subform
                                                                                                      env))
                                                                                            (cddr form)))))))
                        ...
                        (let ((formal-list
                                 (let loop ((formals formals) (acc '()))
                                   (cond
                                     ...
                                     (else (and (identifier? formals) `(,formals . ,acc)))))))
                          (unless formal-list (error "invalid formal arguments" formals))
                            ...
                            (let ((vars
                              ...
                              (let ((body-env (extend-environment '() new-env)))
                                (let-values (((definitions body-exprs)
                                                (let ((body
                                                         (map (lambda (form) (expand form body-env))
                                                              body)))
                                                  (let ()
                                                    ...
                                                    (define (splice-definition definition)
                                                      (cond
                                                        ((ir-define? definition) (list definition))
                                                        ((ir-begin? definition)
                                                            ...
                                                            (unless (and (>= (length form) 4)
                                                                         ...
                                                                         (let ((fields
                                                                           ...
                                                                           (every (lambda (formal)
                                                                                  ...
                                                                                  (cdr (list-ref form
                                                                                                 2)))))
                                                              (error "malformed define-record-type"
                                                                     form))
                                                            (let* ((type (list-ref form 1))
                                                              ...
                                                              (define (field-index tag)
                                                                (let loop ((i 1)
                                                                  ...
                                                                  (cond
                                                                    ((null? field-names)
                                                                      (error "constructor field not found"
                                                                             tag))
                                                                    ((eq? (unwrap-syntax tag)
                                                                          (car field-names))
                                                                      i)
                                                                    (else
                                                                      (loop (+ i 1)
                                                                            (cdr field-names))))))
                                                              (define (constructor-setters record
                                                                       tags)
                                                                (map (lambda (tag)
                                                                       `(,(native-id 'record-set!)
                                                                         ...
                                                                         ,tag))
                                                                     tags))
                                                              (define (expand-field-specs specs
                                                                       index)
                                                                (if (null? specs)
                                                                    '()
                                                                      ...
                                                                      (if (= (length field-spec) 2)
                                                                          (cons `(,(rename 'define)
                                                                                  ...
                                                                                  (,(rename 'lambda)
                                                                                   ...
                                                                                   (,(rename 'if)
                                                                                    (,predicate ,thing)
                                                                                    (,(native-id 'record-ref)
                                                                                     ,thing
                                                                                     ,index)
                                                                                    (,(rename 'error)
                                                                                     "Invalid accessor"))))
                                                                                (expand-field-specs (cdr specs)
                                                                                                    (+ index
                                                                                                       1)))
                                                                          (let ((modifier
                                                                                   (list-ref field-spec
                                                                   ...
                                                                   (,(rename 'lambda)
                                                                    ...
                                                                    (,(rename 'if)
                                                                     ...
                                                                     (,(rename 'eq?)
                                                                      ...
                                                                      ,type)
                                                                     #f)))
                                                                  ,@(expand-field-specs field-specs
                                                                                        1)))))))
                  ...
                  (install-builtin! 'quote
                                    (lambda (form env)
                                      ...
                                      (let ((obj (unwrap-syntax (cadr form)))) (build-quote obj))))

                  (install-builtin! 'if
                                    (lambda (form env)
                                      (case (length form)
                                        ((3)
                                          (build-conditional (expand (cadr form) env)
                  ...
                  (install-builtin! 'begin
                                    (lambda (form env)
                                      (let ((forms (cdr form)))
                                        (build-begin (map (lambda (form) (expand form env)) forms)))))
                    ...
                    (define (interpret-syntax-rules spec spec-env)
                      (er-macro-transformer (lambda (form rename compare)
                                              ...
                                              (define-values (ellipsis ellipsis-env literals rules)
                                                (if (list? (cadr spec))
                                                    (values (make-identifier '... builtin-env)
                                                            builtin-env
                                                            (cadr spec)
                                                            (cddr spec))
                                                    (values (cadr spec)
                                              ...
                                              (define-syntax case-pattern
                                                 (syntax-rules (variable-pattern constant-pattern
                                                                vector-pattern ellipsis-pattern
                                                                pair-pattern)
                                                   ((_ pat
                                                                               ...
                                                                               (let ((new-substs
                                                                                 ...
                                                                                 (let ((base-subst
                                                                                   ...
                                                                                   (append (map (lambda (subst)
                                                                                           ...
                                                                                           (rewrite succ-templ
                                                                                                    #t)))))))
                                                                         (cons (rewrite rep-templ
                                                                                        #f)
                                                                               (rewrite (cdr template)
                                                                                        #f))))
                                                                    ((pair-pattern car-templ
                                                                                   cdr-templ)
