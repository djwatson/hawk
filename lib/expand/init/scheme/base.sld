(define-library (scheme base)
  (export ;; 4.1.2. Literal expressions
    quote
    ;; 4.1.4. Procedures
    lambda
    ;; 4.1.5. Conditionals
  ...
  (begin
    (define-syntax define
       (syntax-rules ()
         ((define (identifier . formals) . body)
          (define identifier (lambda formals . body)))
         ((define identifier expr) (define* identifier expr))))

    (define-syntax cond
       (syntax-rules (else =>)
         ((cond) (if #f #f))
         ((cond (else expr ...)) (begin expr ...))
    ...
    (define-syntax and
       (syntax-rules ()
         ((and) #t)
         ((and form) form)
         ((and form rest ...) (if form (and rest ...) #f))))

    (define-syntax or
       (syntax-rules ()
         ((or) #f)
         ((or form) form)
         ((or form rest ...) (let ((tmp form)) (if tmp tmp (or rest ...))))))

    (define-syntax when
       (syntax-rules () ((when test expr ...) (if test (begin expr ...)))))

    (define-syntax unless
       (syntax-rules () ((unless test expr ...) (when (not test) expr ...))))

    (define-syntax cond-expand ; follows R7RS semantics
       (syntax-rules (else library and or not)
         ((cond-expand) (if #f #f))
         ((cond-expand (else expr ...)) (begin expr ...))
         ((cond-expand ((and) expr ...) clause ...) (begin expr ...))
         ((cond-expand ((and test1 test2 ...) expr ...) clause ...)
          (cond-expand (test1 (cond-expand ((and test2 ...) expr ...) clause ...))
                       clause
                       ...))
         ((cond-expand ((or) expr ...) clause ...) (cond-expand clause ...))
         ((cond-expand ((or test1 test2 ...) expr ...) clause ...)
          (cond-expand (test1 expr ...) ((or test2 ...) expr ...) clause ...))
         ((cond-expand ((not test) expr ...) clause ...)
          (cond-expand (test (cond-expand clause ...)) (else expr ...)))
         ((cond-expand ((library spec) expr ...) clause ...)
          (if-expand (library spec) (begin expr ...) (cond-expand clause ...)))
         ((cond-expand (feature expr ...) clause ...)
          (if-expand feature (begin expr ...) (cond-expand clause ...)))))
    ...
    (define-syntax let-values
      (syntax-rules ()
        ((let-values ((formals0 init0) ...) body0 ...)
         (letrec-syntax ((lv-aux
                          (syntax-rules :::
                            ()
                            ((_ () () bindings . body) (let bindings . body))
    ...
    (define-syntax let*-values
       (syntax-rules ()
         ...
         ((let*-values ((formals init) rest ...) body ...)
          (let-values ((formals init)) (let*-values (rest ...) body ...)))))

    (define-syntax do
       (syntax-rules ()
         ((do ((var init step ...) ...) (test epilogue ...) form ...)
          (let
           loop
           ((var init) ...)
           (if test
         ...
         ((guard (var0 clause0 ...) body0 ...)
          (letrec-syntax ((guard-aux
                             (syntax-rules :::
                               (else =>)
                               ((_ reraise) reraise)
                               ((_ reraise (else expr :::)) (begin expr :::))
                               ((_ reraise (test => proc) clause :::)
                                (let ((tmp test))
                                  (if tmp (proc tmp) (guard-aux reraise clause :::))))
                               ((_ reraise (test) clause :::)
                                (or test (guard-aux reraise clause :::)))
                               ((_ reraise (test expr :::) clause :::)
                                (if test (begin expr :::) (guard-aux reraise clause :::))))))
    ...
    (define-syntax quasiquote ; taken from EIOD
       (syntax-rules (unquote unquote-splicing quasiquote)
         ((quasiquote (quasiquote x) . d) (list 'quasiquote (quasiquote x d)))
         ((quasiquote (unquote x)) x)
         ((quasiquote (unquote x) d) (list 'unquote (quasiquote x . d)))
         ((quasiquote ((unquote-splicing x) . y)) (append x (quasiquote y)))
         ((quasiquote (unquote-splicing x) d)
          (list 'unquote-splicing (quasiquote x . d)))
         ((quasiquote (x . y) . d) (cons (quasiquote x . d) (quasiquote y . d)))
         ((quasiquote #(x ...) . d)
