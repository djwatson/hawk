...
;; (lambda (x) e) → (letrec ([f' (lambda (x) e)]) f')
	  ...
	  (($scletrec)
	   (let ()
	     (define (analyze-group1 g)
	       (define free (fold union '() (map (lambda (f) (cdr (assq (car f) free-table))) g)))
	       (dformat "Analyze group ~a\n" (map car g))
	       (for-each (lambda (f) (set! final-free-table (cons (cons (car f) free) final-free-table))) g))
	     (define again #f)
		 ...
		 (when (not (null? free))
		   (for-each (lambda (f)
			       (define cur-free (assq f final-free-table))
			       (define needs-update (not (memq link (cdr cur-free))))
			       (when needs-update
	  ...
	  (($scletrec)
	   (let ()
		 ...
		 (define (update-lambda f)
		   (define lam (second f))
		   (define foo (dformat "update lam ~a clo ~a\n" lam clo))
		   (if clo
		 ...
		 (define free-replaced (map (lambda (f)
		 ...
		 (define free (cdr (assq (caar g) final-free-table)))
		 (define all-names (apply append (map (lambda (g) (map car g)) (second f))))
		 (define set-names (map car g))
		 (define other-names (difference all-names set-names))
		 (if clo
