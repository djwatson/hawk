  ...
  (define (pass f)
	...
	(case (car f)
	  ...
	  ((letrec)
	   `(letrec ,(map (lambda (f)
			   `(,(car f)
			     (lambda ,(second (second f)) ,@(imap pass (cddr (second f)))))) (second f))
	      ,@(imap pass (cddr f))))
	  ((named-lambda)
	  ...
	  ((scletrec)
	   (let ()
	     ...
	     (define (analyze-group2 g)
	       (define (add-if-needs-closure f link)
		 ...
		 (for-each (lambda (link) (add-if-needs-closure f link)) (difference links g)))
