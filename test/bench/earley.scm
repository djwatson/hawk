...
(define (make-parser grammar lexer)
  ...
  (define (non-terminals grammar) ;; return vector of non-terminals in grammar
    ...
    (define (add-nt nt nts) (if (member nt nts) nts (cons nt nts))) ;; use equal? for equality tests

    (let def-loop ((defs grammar) (nts '()))
      (if (pair? defs)
          (let* ((def (car defs)) (head (car def)))
            (let rule-loop ((rules (cdr def)) (nts (add-nt head nts)))
              (if (pair? rules)
                  (let ((rule (car rules)))
  ...
  (let* ((nts (non-terminals grammar)) ;; id map = list of non-terms
    ...
    (let ((parser-descr (vector lexer nts starters enders predictors steps names)))
      (lambda (input)
        ...
        (define (conf-set-get state conf) (vector-ref state (+ conf 1)))
        ...
        (define (conf-set-head conf-set) (vector-ref conf-set 2))

        (define (conf-set-next conf-set i) (vector-ref conf-set (+ i 5)))
        ...
        (define (conf-set-adjoin* states state-num l i)
          (let ((state (vector-ref states state-num)))
            (let loop ((l1 l))
              (when (pair? l1)
                (let* ((conf (car l1)) (conf-set (conf-set-get* state state-num conf)))
                  (if (not (conf-set-next conf-set i))
                      (begin (conf-set-adjoin state conf-set conf i) (loop (cdr l1)))
                      (loop (cdr l1))))))))
          ...
          (let ((state (vector-ref states state-num)))
            (if (conf-set-member? state conf i)
                (let* ((state* (vector-ref states* state-num))
                  ...
                  (unless (conf-set-next conf-set* i) (conf-set-adjoin state* conf-set* conf i))
                  #t)
                #f)))
        ...
        (define (forw states state-num starters enders predictors steps nts)
          ...
          (define (predict state state-num conf-set conf nt starters enders)
            ...
            (let loop1 ((l (vector-ref starters nt)))
              (when (pair? l)
                (let* ((starter (car l)) (starter-set (conf-set-get* state state-num starter)))
                  (if (not (conf-set-next starter-set state-num))
                      (begin (conf-set-adjoin state starter-set starter state-num) (loop1 (cdr l)))
                      (loop1 (cdr l))))))
              ...
              (when (pair? l)
                (let ((ender (car l)))
                  (if (conf-set-member? state ender state-num)
                      (let* ((next (+ conf 1)) (next-set (conf-set-get* state state-num next)))
          ...
          (let ((state (vector-ref states state-num)) (nb-nts (vector-length nts)))
            (let loop ()
          ...
          (let ((prev (- conf 1)))
            (when (and (>= conf nb-nts) (>= (vector-ref steps prev) 0))
              (let loop1 ((l (vector-ref enders (vector-ref steps prev))))
                (when (pair? l)
                  (let* ((ender (car l)) (ender-set (conf-set-get (vector-ref states j) ender)))
                    (if ender-set
        ...
        (define (back states states* state-num enders steps nb-nts toks)
          (let ((state* (vector-ref states* state-num)))
            (let loop1 ()
              (let ((conf (vector-ref state* 0)))
                (when (>= conf 0)
                  (let* ((conf-set (vector-ref state* (+ conf 1))) (head (vector-ref conf-set 4)))
                    (vector-set! state* 0 (vector-ref conf-set 0))
                    (conf-set-merge-new! conf-set)
                    (let loop2 ((i head))
                      (if (>= i 0)
        ...
        (define (deriv-trees conf i j enders steps names toks states nb-nts)
          (let ((name (vector-ref names conf)))
            ...
            (if name ;; `conf' is at the start of a rule (either special or not)
                ...
                (let ((prev (- conf 1)))
                  (let loop1 ((l1 (vector-ref enders (vector-ref steps prev))) (l2 '()))
                    (if (pair? l1)
                        (let* ((ender (car l1))
                          ...
                          (if ender-set
                              (let loop2 ((k (conf-set-head ender-set)) (l2 l2))
                                (if (>= k 0)
                                    (if (and (>= k i)
                                             (conf-set-member? (vector-ref states k) prev i))
                                        (let ((prev-trees
                                                 (deriv-trees prev
                                                              i
                                                              k
        ...
        (define (nb-deriv-trees conf i j enders steps toks states nb-nts)
          (let ((prev (- conf 1)))
            (if (or (< conf nb-nts) (< (vector-ref steps prev) 0))
                1
                (let loop1 ((l (vector-ref enders (vector-ref steps prev))) (n 0))
                  (if (pair? l)
                      (let* ((ender (car l)) (ender-set (conf-set-get (vector-ref states j) ender)))
                        (if ender-set
...
(define (test input)
  (let ((p
           (make-parser '((s (a) (s s))) (lambda (l) (map (lambda (x) (list x x)) l)))))
    (let ((x (p input)) (n (length input))) (length (parse->trees x 's 0 n)))))
