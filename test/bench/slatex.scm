...
(define slatex.ormapcdr
  (lambda (f l) (let loop ((l l)) (if (null? l) #f (or (f l) (loop (cdr l)))))))
...
(define slatex.append!
  (lambda (l1 l2)
    (cond
      ...
      ((null? l2) l1)
      (else
        (let loop ((l1 l1)) (if (null? (cdr l1)) (set-cdr! l1 l2) (loop (cdr l1))))
        l1))))
...
(define slatex.list-set!
  (lambda (l i v)
    (let loop ((l l) (i i))
      (cond
        ((null? l) (slatex.error 'slatex.list-set! 'list-too-small))
        ((= i 0) (set-car! l v))
        (else (loop (cdr l) (- i 1)))))))
    ...
    (let ((pfx-len (string-length pfx)) (s-len (string-length s)))
      (if (> pfx-len s-len)
          ...
          (let loop ((i 0))
            (if (>= i pfx-len)
                #t
                (and (char=? (string-ref pfx i) (string-ref s i)) (loop (+ i 1)))))))))
    ...
    (let ((sfx-len (string-length sfx)) (s-len (string-length s)))
      (if (> sfx-len s-len)
          ...
          (let loop ((i (- sfx-len 1)) (j (- s-len 1)))
            (if (< i 0)
                #t
                (and (char=? (string-ref sfx i) (string-ref s j)) (loop (- i 1) (- j 1)))))))))
...
(define slatex.position-char
  (lambda (c l)
    (let loop ((l l) (i 0))
      (cond ((null? l) #f) ((char=? (car l) c) i) (else (loop (cdr l) (+ i 1)))))))

(define slatex.string-position-right
  (lambda (c s)
    (let ((n (string-length s)))
      (let loop ((i (- n 1)))
        (cond ((< i 0) #f) ((char=? (string-ref s i) c) i) (else (loop (- i 1))))))))
...
(define slatex.member-token
  (lambda (x s)
    (slatex.ormapcdr (lambda (s_i..) (if (slatex.token=? (car s_i..) x) s_i.. #f))
                     s)))
...
(define slatex.set-keyword
  (lambda (x)
    (if (slatex.member-token x slatex.keyword-tokens)
        'skip
        (begin
          (set! slatex.constant-tokens (slatex.remove-token! x slatex.constant-tokens))
          (set! slatex.variable-tokens (slatex.remove-token! x slatex.variable-tokens))
...
(define slatex.set-constant
  (lambda (x)
    (if (slatex.member-token x slatex.constant-tokens)
        'skip
        (begin
          (set! slatex.keyword-tokens (slatex.remove-token! x slatex.keyword-tokens))
          (set! slatex.variable-tokens (slatex.remove-token! x slatex.variable-tokens))
...
(define slatex.set-special-symbol
  (lambda (x transl)
    (let ((c (slatex.assoc-token x slatex.special-symbols)))
      (if c
          (set-cdr! c transl)
          (set! slatex.special-symbols (cons (cons x transl) slatex.special-symbols))))))

(define slatex.unset-special-symbol
  (lambda (x)
    (set! slatex.special-symbols
      (slatex.remove-if! (lambda (c) (slatex.token=? (car c) x))
  ...
  (lambda (s)
    (let loop ((l (slatex.texify-aux s)) (r '()))
      (if (null? l)
          ...
          (let ((c (car l)))
            (loop (cdr l)
                  (if (char=? c #\-) (slatex.append! (list #\$ c #\$) r) (cons c r))))))))
  ...
  (let* ((arrow (string->list "-$>$")) (arrow-lh (length arrow)))
    (lambda (s)
      (let* ((sl (string->list s))
             (texified-sl
                (slatex.append-map! (lambda (c) (string->list (slatex.tex-analog c))) sl)))
        (slatex.ormapcdr (lambda (d)
...
(define slatex.display-begin-sequence
  (lambda (out)
    (if (or slatex.*intext?* (not slatex.*latex?*))
        (begin (display "\\" out) (display slatex.*code-env-spec* out) (newline out))
        (begin
          (display "\\begin{" out)
...
(define slatex.make-line
  (lambda ()
    (let ((l (slatex.make-raw-line)))
      ...
      (vector-set! l slatex.=char (make-string slatex.*max-line-length* #\space))
      (vector-set! l
                   slatex.=space
...
(define slatex.display-space
  (lambda (s p)
    (cond
      ((eqv? s slatex.&plain-space) (display #\space p))
...
(define slatex.display-tab
  (lambda (tab p)
    (cond
      ((eqv? tab slatex.&set-tab) (display "\\=" p))
      ((eqv? tab slatex.&move-tab) (display "\\>" p)))))

(define slatex.display-notab
  (lambda (notab p)
    (cond
      ((eqv? notab slatex.&begin-string) (display "\\dt{" p))
      ...
      (let ((graphic-char-seen? #f))
        (let loop ((i 0))
          (let ((c (read-char slatex.*in*)))
            (cond
              (graphic-char-seen? 'already-seen)
              ((or (eof-object? c)
            ...
            (cond
              ((eof-object? c)
                (cond
                  ((eqv? curr-notab slatex.&mid-string)
                    (if (> i 0)
                        (string-set! (vector-ref line slatex.=notab) (- i 1) slatex.&end-string)))
                  ((eqv? curr-notab slatex.&mid-comment) (set! curr-notab slatex.&void-notab))
                  ((eqv? curr-notab slatex.&mid-math)
                    (slatex.error 'slatex.get-line 'runaway-math-subformula)))
                (string-set! (vector-ref line slatex.=char) i #\newline)
                (string-set! (vector-ref line slatex.=space) i slatex.&void-space)
                (string-set! (vector-ref line slatex.=tab) i slatex.&void-tab)
                (string-set! (vector-ref line slatex.=notab) i slatex.&void-notab)
                (vector-set! line slatex.=rtedge i)
                (if (eqv? (string-ref (vector-ref line slatex.=notab) 0) slatex.&mid-string)
                    (string-set! (vector-ref line slatex.=notab) 0 slatex.&begin-string))
                (if (= i 0) #f #t))
              ((or (char=? c slatex.*return*) (char=? c #\newline))
                (if (and (eq? *op-sys* 'dos) (char=? c slatex.*return*))
                    (if (char=? (peek-char slatex.*in*) #\newline) (read-char slatex.*in*)))
                (cond
                  ((eqv? curr-notab slatex.&mid-string)
                    (if (> i 0)
                        (string-set! (vector-ref line slatex.=notab) (- i 1) slatex.&end-string)))
                ...
                (string-set! (vector-ref line slatex.=char) i #\newline)
                (string-set! (vector-ref line slatex.=space) i slatex.&void-space)
                (string-set! (vector-ref line slatex.=tab)
                             i
                             (cond
                               ((eof-object? (peek-char slatex.*in*)) slatex.&plain-crg-ret)
              ...
              ((eqv? curr-notab slatex.&mid-comment)
                ...
                (string-set! (vector-ref line slatex.=space)
                             i
                             (cond
                               ((char=? c #\space) slatex.&plain-space)
                               ((char=? c slatex.*tab*) slatex.&plain-space)
                               (else slatex.&void-space)))
                (string-set! (vector-ref line slatex.=tab) i slatex.&void-tab)
                (string-set! (vector-ref line slatex.=notab) i slatex.&mid-comment)
                (loop (+ i 1)))
              ((char=? c #\\)
                (string-set! (vector-ref line slatex.=char) i c)
                (string-set! (vector-ref line slatex.=space) i slatex.&void-space)
                ...
                (let ((i+1 (+ i 1)) (c+1 (read-char slatex.*in*)))
                  ...
                  (string-set! (vector-ref line slatex.=space)
                               i+1
                               (if (char=? c+1 #\space) slatex.&plain-space slatex.&void-space))
                  (string-set! (vector-ref line slatex.=tab) i+1 slatex.&void-tab)
                  (string-set! (vector-ref line slatex.=notab) i+1 curr-notab)
                  (loop (+ i+1 1))))
              ((eqv? curr-notab slatex.&mid-math)
                (if (char=? c slatex.*tab*) (set! c #\space))
                (string-set! (vector-ref line slatex.=space)
                             i
                             (if (char=? c #\space) slatex.&plain-space slatex.&void-space))
                (string-set! (vector-ref line slatex.=tab) i slatex.&void-tab)
                (cond
                  ((memv c slatex.*math-triggerers*)
                    (string-set! (vector-ref line slatex.=char) i #\$)
                    (string-set! (vector-ref line slatex.=notab) i slatex.&end-math)
                    (set! curr-notab slatex.&void-notab))
                  (else
                    (string-set! (vector-ref line slatex.=char) i c)
                    (string-set! (vector-ref line slatex.=notab) i slatex.&mid-math)))
                (loop (+ i 1)))
              ((eqv? curr-notab slatex.&mid-string)
                (if (char=? c slatex.*tab*) (set! c #\space))
                (string-set! (vector-ref line slatex.=char) i c)
                (string-set! (vector-ref line slatex.=space)
                             i
                             (if (char=? c #\space) slatex.&inner-space slatex.&void-space))
                (string-set! (vector-ref line slatex.=tab) i slatex.&void-tab)
                (string-set! (vector-ref line slatex.=notab)
                             i
                             (cond
                               ((char=? c #\")
                                 (set! curr-notab slatex.&void-notab)
              ...
              ((char=? c #\space)
                ...
                (string-set! (vector-ref line slatex.=space)
                             i
                             (cond
                               (slatex.*intext?* slatex.&plain-space)
              ...
              ((char=? c #\")
                ...
                (string-set! (vector-ref line slatex.=space) i slatex.&void-space)
                (string-set! (vector-ref line slatex.=tab) i slatex.&void-tab)
                (string-set! (vector-ref line slatex.=notab) i slatex.&begin-string)
              ...
              ((char=? c #\;)
                ...
                (string-set! (vector-ref line slatex.=space) i slatex.&void-space)
                (string-set! (vector-ref line slatex.=tab) i slatex.&void-tab)
                (string-set! (vector-ref line slatex.=notab) i slatex.&begin-comment)
              ...
              ((memv c slatex.*math-triggerers*)
                ...
                (string-set! (vector-ref line slatex.=space) i slatex.&void-space)
                (string-set! (vector-ref line slatex.=tab) i slatex.&void-tab)
                (string-set! (vector-ref line slatex.=notab) i slatex.&begin-math)
                  ...
                  (begin
                    ...
                    (let ((prev-rtedge (vector-ref prev slatex.=rtedge)))
                      (if (eqv? (string-ref (vector-ref prev slatex.=tab) prev-rtedge)
                          ...
                          (string-set! (vector-ref prev slatex.=tab)
          ...
          (if slatex.*latex-paragraph-mode?*
              ...
              (if slatex.*intext?*
                  ...
                  (let ((remove-tabs-from #f))
                    (let loop ((i 0))
                      (cond
                        ((char=? (string-ref (vector-ref curr slatex.=char) i) #\newline)
                          (set! remove-tabs-from i))
                        ((char=? (string-ref (vector-ref prev slatex.=char) i) #\newline)
                          (set! remove-tabs-from #f))
                        ((eqv? (string-ref (vector-ref curr slatex.=space) i) slatex.&init-space)
                          (if (eqv? (string-ref (vector-ref prev slatex.=notab) i)
                                    slatex.&void-notab)
                              (begin
                                (cond
                                  ((or (char=? (string-ref (vector-ref prev slatex.=char) i) #\()
                                      (eqv? (string-ref (vector-ref prev slatex.=space) i)
                                            slatex.&paren-space))
                                    (string-set! (vector-ref curr slatex.=space)
                                                 i
                                                 slatex.&paren-space))
                                  ((or (char=? (string-ref (vector-ref prev slatex.=char) i) #\[)
                                      (eqv? (string-ref (vector-ref prev slatex.=space) i)
                                    ...
                                    (string-set! (vector-ref curr slatex.=space)
                                                 i
                                                 slatex.&bracket-space))
                                  ((or (memv (string-ref (vector-ref prev slatex.=char) i)
                                             '(#\' #\` #\,))
                                      (eqv? (string-ref (vector-ref prev slatex.=space) i)
                                            slatex.&quote-space))
                                    (string-set! (vector-ref curr slatex.=space)
                                ...
                                (if (memv (string-ref (vector-ref prev slatex.=tab) i)
                                    ...
                                    (string-set! (vector-ref curr slatex.=tab) i slatex.&move-tab))))
                          (loop (+ i 1)))
                        ((= i 0) (set! remove-tabs-from 0))
                        ((not (eqv? (string-ref (vector-ref prev slatex.=tab) i) slatex.&void-tab))
                        ...
                        ((memv (string-ref (vector-ref prev slatex.=space) i)
                               (list slatex.&init-space
                                     ...
                                     slatex.&quote-space))
                          (set! remove-tabs-from (+ i 1)))
                        ((and (char=? (string-ref (vector-ref prev slatex.=char) (- i 1)) #\space)
                              (eqv? (string-ref (vector-ref prev slatex.=notab) (- i 1))
                                    slatex.&void-notab))
                          (set! remove-tabs-from (+ i 1))
                          (string-set! (vector-ref prev slatex.=tab) i slatex.&set-tab)
                          (string-set! (vector-ref curr slatex.=tab) i slatex.&move-tab))
                        (else
                          (set! remove-tabs-from (+ i 1))
                          (let loop1 ((j (- i 1)))
                            (cond
                              ((<= j 0) 'exit-loop1)
                              ((not (eqv? (string-ref (vector-ref curr slatex.=tab) j)
                                          slatex.&void-tab))
                                'exit-loop1)
                              ((memv (string-ref (vector-ref curr slatex.=space) j)
                                     (list slatex.&paren-space
                                           slatex.&bracket-space
                                           slatex.&quote-space))
                                (loop1 (- j 1)))
                              ((or (not (eqv? (string-ref (vector-ref prev slatex.=notab) j)
                                              slatex.&void-notab))
                                  (char=? (string-ref (vector-ref prev slatex.=char) j) #\space))
                                (let ((k (+ j 1)))
                                  (if (memv (string-ref (vector-ref prev slatex.=notab) k)
                                      ...
                                      'skip
                                      (begin
                                        (if (eqv? (string-ref (vector-ref prev slatex.=tab) k)
                                                  slatex.&void-tab)
                                            (string-set! (vector-ref prev slatex.=tab)
                                                         k
                                                         slatex.&set-tab))
                                        (string-set! (vector-ref curr slatex.=tab)
                                                     k
                                                     slatex.&move-tab)))))
                              (else 'anything-else?))))))
  ...
  (lambda (line)
    (let loop ((i 1) (succ-parens? #f))
      (let ((c (string-ref (vector-ref line slatex.=char) i)))
        (cond
          ((char=? c #\newline) 'exit-loop)
          ((not (eqv? (string-ref (vector-ref line slatex.=notab) i) slatex.&void-notab))
            (loop (+ i 1) #f))
          ((char=? c #\[)
            (if (eqv? (string-ref (vector-ref line slatex.=tab) i) slatex.&void-tab)
...
(define slatex.remove-some-tabs
  (lambda (line i)
    (if i
        (let loop ((i i))
          (cond
            ((char=? (string-ref (vector-ref line slatex.=char) i) #\newline) 'exit)
            ((eqv? (string-ref (vector-ref line slatex.=tab) i) slatex.&set-tab)
              (string-set! (vector-ref line slatex.=tab) i slatex.&void-tab)
              (loop (+ i 1)))
            (else (loop (+ i 1))))))))
...
(define slatex.clean-init-spaces
  (lambda (line)
    (let loop ((i (vector-ref line slatex.=rtedge)))
      (cond
        ((< i 0) 'exit-loop)
        ((eqv? (string-ref (vector-ref line slatex.=tab) i) slatex.&move-tab)
          (let loop2 ((i (- i 1)))
            (cond
              ((< i 0) 'exit-loop2)
              ((memv (string-ref (vector-ref line slatex.=space) i)
                     (list slatex.&init-space
                           ...
                           slatex.&bracket-space
                           slatex.&quote-space))
                (string-set! (vector-ref line slatex.=space) i slatex.&init-plain-space)
                (loop2 (- i 1)))
              (else (loop2 (- i 1))))))
        (else (loop (- i 1)))))))
...
(define slatex.clean-inner-spaces
  (lambda (line)
    (let loop ((i 0) (succ-inner-spaces? #f))
      (cond
        ((char=? (string-ref (vector-ref line slatex.=char) i) #\newline) 'exit-loop)
        ((eqv? (string-ref (vector-ref line slatex.=space) i) slatex.&inner-space)
          (if succ-inner-spaces?
              'skip
              (string-set! (vector-ref line slatex.=space) i slatex.&plain-space))
          (loop (+ i 1) #t))
        (else (loop (+ i 1) #f))))))
  ...
  (lambda (line)
    (let loop ((i 0))
      (let ((c (string-ref (vector-ref line slatex.=char) i)))
        (cond
          ((char=? c #\space)
            (if (eqv? (string-ref (vector-ref line slatex.=notab) i) slatex.&void-notab)
                (loop (+ i 1))
                #f))
          ((char=? c #\newline)
            (let loop2 ((j (- i 1)))
...
(define slatex.flush-comment-line?
  (lambda (line)
    (and (char=? (string-ref (vector-ref line slatex.=char) 0) #\;)
         (eqv? (string-ref (vector-ref line slatex.=notab) 0) slatex.&begin-comment)
  ...
  (lambda ()
    (let loop ((line1 slatex.*line1*) (line2 slatex.*line2*))
      (let* ((line2-paragraph? slatex.*latex-paragraph-mode?*)
        ...
        ((if line2-paragraph? slatex.display-tex-line slatex.display-scm-line) line2)
  ...
  (lambda (inport outport)
    ...
    (let ((flush-line
             (lambda (line)
               ...
               (string-set! (vector-ref line slatex.=space) 0 slatex.&void-space)
               (string-set! (vector-ref line slatex.=tab) 0 slatex.&void-tab)
               (string-set! (vector-ref line slatex.=notab) 0 slatex.&void-notab))))
            ...
            (let ((tab (string-ref (vector-ref line slatex.=tab) i)))
              (cond
                ...
                ((eqv? tab slatex.&void-tab)
                  (display #\% slatex.*out*)
                  (newline slatex.*out*)))))
          ((eqv? (string-ref (vector-ref line slatex.=notab) i) slatex.&begin-comment)
            (slatex.display-tab (string-ref (vector-ref line slatex.=tab) i) slatex.*out*)
            (display c slatex.*out*)
            (loop (+ i 1)))
          ((eqv? (string-ref (vector-ref line slatex.=notab) i) slatex.&mid-comment)
            (display c slatex.*out*)
            (loop (+ i 1)))
          ((eqv? (string-ref (vector-ref line slatex.=notab) i) slatex.&begin-string)
            (slatex.display-tab (string-ref (vector-ref line slatex.=tab) i) slatex.*out*)
            (display "\\dt{" slatex.*out*)
            (if (char=? c #\space)
                (slatex.display-space (string-ref (vector-ref line slatex.=space) i)
                                      slatex.*out*)
                (slatex.display-tex-char c slatex.*out*))
            (loop (+ i 1)))
          ((eqv? (string-ref (vector-ref line slatex.=notab) i) slatex.&mid-string)
            (if (char=? c #\space)
                (slatex.display-space (string-ref (vector-ref line slatex.=space) i)
                                      slatex.*out*)
                (slatex.display-tex-char c slatex.*out*))
            (loop (+ i 1)))
          ((eqv? (string-ref (vector-ref line slatex.=notab) i) slatex.&end-string)
            (if (char=? c #\space)
                (slatex.display-space (string-ref (vector-ref line slatex.=space) i)
                                      slatex.*out*)
                (slatex.display-tex-char c slatex.*out*))
            (display "}" slatex.*out*)
            (loop (+ i 1)))
          ((eqv? (string-ref (vector-ref line slatex.=notab) i) slatex.&begin-math)
            (slatex.display-tab (string-ref (vector-ref line slatex.=tab) i) slatex.*out*)
            (display c slatex.*out*)
            (loop (+ i 1)))
          ...
          ((memv c '(#\( #\[))
            ...
            (if (null? slatex.*bq-stack*)
                ...
                (let ((top (car slatex.*bq-stack*)))
                  (cond
                    ((vector-ref top slatex.=in-bq-tkn)
                ...
                (let ((top (car slatex.*case-stack*)))
                  (cond
                    ...
                    ((> (vector-ref top slatex.=in-case-exp) 0)
                      ...
                      (if (= (vector-ref top slatex.=in-case-exp) 2) (set! slatex.*in-qtd-tkn* #t))))))
            (loop (+ i 1)))
          ((memv c '(#\) #\]))
            (slatex.display-tab (string-ref (vector-ref line slatex.=tab) i) slatex.*out*)
            (display c slatex.*out*)
                    ...
                    (cond
                      ...
                      ((> (vector-ref top slatex.=in-case-exp) 0)
                        ...
                        (if (= (vector-ref top slatex.=in-case-exp) 0)
                            (begin (set! slatex.*case-stack* (cdr slatex.*case-stack*)) (loop))))))))
            (loop (+ i 1)))
          (else
            (slatex.display-tab (string-ref (vector-ref line slatex.=tab) i) slatex.*out*)
    ...
    (lambda (line i)
      (let loop ((buf '()) (i i))
        (let ((c (string-ref (vector-ref line slatex.=char) i)))
          (cond
            ((char=? c #\\)
              (loop (cons (string-ref (vector-ref line slatex.=char) (+ i 1)) (cons c buf))
                    (+ i 2)))
            ((or (memv c token-delims) (memv c slatex.*math-triggerers*))
              (slatex.output-token (list->string (slatex.reverse! buf)))
              i)
            ((char? c)
              (loop (cons (string-ref (vector-ref line slatex.=char) i) buf) (+ i 1)))
...
(define slatex.output-token
  (lambda (token)
    ...
    (if (slatex.assoc-token token slatex.special-symbols)
        (display (cdr (slatex.assoc-token token slatex.special-symbols)) slatex.*out*)
        (slatex.display-token token
                              (cond
                                (slatex.*in-qtd-tkn*
                                  (set! slatex.*in-qtd-tkn* #f)
                                  (cond
                                ...
                                ((and (not (null? slatex.*bq-stack*))
                                      (not (vector-ref (car slatex.*bq-stack*) slatex.=in-comma)))
                                  'constant)
                                (slatex.*in-mac-tkn*
                                  (set! slatex.*in-mac-tkn* #f)
...
(define slatex.data-token?
  (lambda (token) (or (char=? (string-ref token 0) #\#) (string->number token))))
...
(define slatex.*file-hider*
  (cond ((eq? *op-sys* 'unix) "outputs/") ((eq? *op-sys* 'dos) "x") (else ".")))
          ...
          (let ((dir (car path)))
            (let loop2 ((files
                           (if (or (string=? dir "") (string=? dir "."))
                               files
                               (map (lambda (file) (string-append dir slatex.*directory-mark* file))
                                    files))))
              (if (null? files)
                  (loop (cdr path))
                  (let ((file (car files)))
                    (if (slatex.file-exists? file) file (loop2 (cdr files)))))))))))
  ...
  (lambda (filename)
    (let ((extn (slatex.file-extension filename)))
      (if (and extn (or (string=? extn ".sty") (string=? extn ".tex")))
          ...
          (slatex.find-some-file slatex.*texinputs-list*
...
(define slatex.full-scmfile-name
  (lambda (filename)
    (apply slatex.find-some-file
           ...
           filename
           (map (lambda (extn) (string-append filename extn)) '(".scm" ".ss" ".s")))))
...
(define slatex.eat-till-newline
  (lambda (in)
    (let loop ()
      (let ((c (read-char in)))
        (cond ((eof-object? c) 'done) ((char=? c #\newline) 'done) (else (loop)))))))
  ...
  (lambda (in)
    (let ((c (read-char in)))
      ...
      (if (char-alphabetic? c)
          (list->string (slatex.reverse! (let loop ((s (list c)))
                                           (let ((c (peek-char in)))
                                             (cond
                                               ((eof-object? c) s)
                                               ((char-alphabetic? c)
  ...
  (lambda (in)
    (let loop ()
      (let ((c (peek-char in)))
        (cond
          ((eof-object? c) 'done)
          ((or (char=? c #\space) (char=? c slatex.*tab*)) (read-char in) (loop))
          (else 'done))))))
...
(define slatex.read-grouped-latexexp
  (lambda (in)
    ...
    (let ((c (read-char in)))
      ...
      (slatex.eat-latex-whitespace in)
      (list->string (slatex.reverse! (slatex.chop-off-whitespace (let loop ((s '())
                                                                            (nesting 0)
                                                                            (escape? #f))
                                                                   (let ((c (read-char in)))
                                                                     (if (eof-object? c)
                                                                         (slatex.error 'slatex.read-grouped-latexexp
                                                                                       3))
                                                                     (cond
                                                                       (escape?
                                                                         (loop (cons c s)
    ...
    (lambda (in)
      ...
      (let ((c (peek-char in)))
        ...
        (if (char=? c #\{)
            ...
            (list->string (slatex.reverse! (let loop ((s '()) (escape? #f))
                                             (let ((c (peek-char in)))
                                               (cond
                                                 ((eof-object? c)
                                                   (if escape?
...
(define slatex.read-delimed-commaed-filenames
  (lambda (in lft-delim rt-delim)
    (slatex.eat-latex-whitespace in)
    (let ((c (read-char in)))
      (if (eof-object? c) (slatex.error 'slatex.read-delimed-commaed-filenames 1))
      (if (char=? c lft-delim)
          'ok
      ...
      (let loop ((s '()))
        (slatex.eat-latex-whitespace in)
        (let ((c (peek-char in)))
          (if (eof-object? c) (slatex.error 'slatex.read-delimed-commaed-filenames 3))
          (if (char=? c rt-delim)
              (begin (read-char in) (slatex.reverse! s))
              (let ((s (cons (slatex.read-filename in) s)))
                (slatex.eat-latex-whitespace in)
                (let ((c (peek-char in)))
                  (if (eof-object? c) (slatex.error 'slatex.read-delimed-commaed-filenames 4))
                  (cond
                    ((char=? c #\,) (read-char in))
                    ((char=? c rt-delim) 'void)
                    (else (slatex.error 'slatex.read-delimed-commaed-filenames 5)))
                  (loop s)))))))))
  ...
  (lambda (in categ)
    (let ((setter
             (cond
               ...
               (else (slatex.error 'slatex.add-to-slatex-db-basic 1))))
...
(define slatex.add-to-slatex-db-special
  (lambda (in what)
    (let ((ids (slatex.read-grouped-schemeids in)))
      (cond
        ((eq? what 'unsetspecialsymbol) (for-each slatex.unset-special-symbol ids))
        ((eq? what 'setspecialsymbol)
          (if (= (length ids) 1)
              'ok
              (slatex.error 'slatex.add-to-slatex-db-special
...
(define slatex.process-slatex-alias
  (lambda (in what which)
    (let ((triggerer (slatex.read-grouped-latexexp in)))
      (cond
        ((eq? which 'intext)
          (set! slatex.*intext-triggerers* (what triggerer slatex.*intext-triggerers*)))
        ...
        ((eq? which 'box)
          (set! slatex.*box-triggerers* (what triggerer slatex.*box-triggerers*)))
        ((eq? which 'input)
          (set! slatex.*input-triggerers* (what triggerer slatex.*input-triggerers*)))
        ((eq? which 'region)
          (set! slatex.*region-triggerers* (what triggerer slatex.*region-triggerers*)))
        ((eq? which 'mathescape)
          (if (= (string-length triggerer) 1)
              'ok
              (slatex.error 'slatex.process-slatex-alias 'math-escape-should-be-character))
    ...
    (if (char=? (peek-char in) #\[)
        (for-each (lambda (filename)
                    (let ((%:g0% slatex.*slatex-in-protected-region?*))
                      ...
                      (let ((%temp%
                               (begin (slatex.process-tex-file (string-append filename ".sty")))))
    ...
    (let ((bool (slatex.read-grouped-latexexp in)))
      (set! slatex.*slatex-case-sensitive?*
        (cond
          ...
          (else (slatex.error 'slatex.process-case-info 'bad-schemecasesensitive-arg)))))))
...
(define slatex.process-main-tex-file
  (lambda (filename)
    ...
    (let ((file-hide-file "xZfilhid.tex"))
      (if (slatex.file-exists? file-hide-file) (slatex.delete-file file-hide-file))
      (if (eq? *op-sys* 'dos)
...
(define slatex.process-tex-file
  (lambda (raw-filename)
    (if slatex.debug? (begin (display "begin ") (display raw-filename) (newline)))
                              ...
                              (let ((cs (slatex.read-ctrl-seq in)))
                                ...
                                (cond
                                  ...
                                  ((member cs slatex.*input-triggerers*)
                                    (slatex.process-scheme-file (slatex.read-filename in)))
                                  ((string=? cs "input")
                                    (let ((%:g1% slatex.*slatex-in-protected-region?*))
                                      (set! slatex.*slatex-in-protected-region?* #f)
                                      (let ((%temp%
                                               (begin
                                                 (slatex.process-tex-file (slatex.read-filename in)))))
                                        (set! slatex.*slatex-in-protected-region?* %:g1%)
                                        %temp%)))
                                  ((string=? cs "include")
                                    (if slatex.*latex?*
                                        (let ((f
                                                 (slatex.full-texfile-name (slatex.read-filename in))))
                                          (if (and f
                                                   (or (eq? slatex.*include-onlys* 'all)
                                                      (member f slatex.*include-onlys*)))
                                              (let ((%:g2% slatex.*slatex-in-protected-region?*)
                                                    (%:g3% slatex.subjobname)
                                                    (%:g4% primary-aux-file-count))
                                                (set! slatex.*slatex-in-protected-region?* #f)
                                                (set! slatex.subjobname slatex.subjobname)
                                                (set! primary-aux-file-count primary-aux-file-count)
                                                (let ((%temp%
                                                         (begin
                                                           (if slatex.*slatex-separate-includes?*
                                                               (begin
                                                                 (set! slatex.subjobname
                                                                   (slatex.basename f ".tex"))
                                                                 (set! primary-aux-file-count -1)))
                                                           (slatex.process-tex-file f))))
                                                  (set! slatex.*slatex-in-protected-region?* %:g2%)
                                                  (set! slatex.subjobname %:g3%)
                                                  (set! primary-aux-file-count %:g4%)
                                                  %temp%))))))
                                  ((string=? cs "includeonly")
                                    (if slatex.*latex?* (slatex.process-include-only in)))
                                  ((string=? cs "documentstyle")
                                    (if slatex.*latex?* (slatex.process-documentstyle in)))
                                  ((string=? cs "schemecasesensitive")
                                    (slatex.process-case-info in))
                                  ((string=? cs "defschemetoken")
                                    (slatex.process-slatex-alias in slatex.adjoin-string 'intext))
                                  ((string=? cs "undefschemetoken")
                                    (slatex.process-slatex-alias in slatex.remove-string! 'intext))
                                  ((string=? cs "defschemeresulttoken")
                                    (slatex.process-slatex-alias in
                                  ...
                                  ((string=? cs "defschemedisplaytoken")
                                    (slatex.process-slatex-alias in slatex.adjoin-string 'display))
                                  ((string=? cs "undefschemedisplaytoken")
                                    (slatex.process-slatex-alias in slatex.remove-string! 'display))
                                  ((string=? cs "defschemeboxtoken")
                                    (slatex.process-slatex-alias in slatex.adjoin-string 'box))
                                  ((string=? cs "undefschemeboxtoken")
                                    (slatex.process-slatex-alias in slatex.remove-string! 'box))
                                  ((string=? cs "defschemeinputtoken")
                                    (slatex.process-slatex-alias in slatex.adjoin-string 'input))
                                  ((string=? cs "undefschemeinputtoken")
                                    (slatex.process-slatex-alias in slatex.remove-string! 'input))
                                  ((string=? cs "defschemeregiontoken")
                                    (slatex.process-slatex-alias in slatex.adjoin-string 'region))
                                  ((string=? cs "undefschemeregiontoken")
                                    (slatex.process-slatex-alias in slatex.remove-string! 'region))
                                  ((string=? cs "defschememathescape")
                                    (slatex.process-slatex-alias in slatex.adjoin-char 'mathescape))
                                  ((string=? cs "undefschememathescape")
                                    (slatex.process-slatex-alias in slatex.remove-char! 'mathescape))
                                  ((string=? cs "setkeyword") (slatex.add-to-slatex-db in 'keyword))
                                  ...
                                  ((string=? cs "unsetspecialsymbol")
                                    (slatex.add-to-slatex-db in 'unsetspecialsymbol)))))))
                        (loop)))))))))
    (if slatex.debug? (begin (display "end ") (display raw-filename) (newline)))))
      ...
      (if (not filename)
          ...
          (let ((aux.tex (slatex.new-aux-file ".tex")))
            ...
            (call-with-input-file filename
              (lambda (in)
                (call-with-output-file aux.tex
                  (lambda (out)
                    (let ((%:g5% slatex.*intext?*) (%:g6% slatex.*code-env-spec*))
                      (set! slatex.*intext?* #f)
            ...
            (if slatex.*slatex-in-protected-region?*
                (set! slatex.*protected-files* (cons aux.tex slatex.*protected-files*)))
            (slatex.process-tex-file filename))))))
      ...
      (let ((%:g9% slatex.*slatex-in-protected-region?*)
        ...
        (let ((%temp%
                 (begin
                   ...
                   (set! slatex.*protected-files* (slatex.reverse! slatex.*protected-files*))
      ...
      (let loop ()
        (if done?
            ...
            (begin
              (let ((c (read-char in)))
                (cond
                  ((eof-object? c) (display "{}" out) (set! done? #t))
                  ((char=? c #\%) (slatex.eat-till-newline in))
                  ((char=? c #\\)
                    (let ((cs (slatex.read-ctrl-seq in)))
                      (cond
                        ((string=? cs "begin")
                          (let ((cs (slatex.read-grouped-latexexp in)))
                            (cond
                              ...
                              (else (display "\\begin{" out) (display cs out) (display "}" out)))))
                        ((member cs slatex.*intext-triggerers*)
    ...
    (cond
      ...
      (else
        (let ((f (car slatex.*protected-files*)))
          ...
          (call-with-input-file f (lambda (in) (slatex.inline-protected-files in out)))
          (slatex.delete-file f))
        (cond
          ((memq typ '(intext resultintext)) (slatex.dump-intext in #f))
          ((memq typ '(envdisplay envbox))
...
(define (main)
  (let* ((count 500)
         (input1 "inputs/slatex-data/test")
         (input2 'ignored)
         (output 'ignored)
