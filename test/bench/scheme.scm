...
(define (scheme-error msg . args) (error #f msg args))
...
(define (variable x)
  (unless (symbol? x) (scheme-error "Identifier expected" x))
...
(define (comp-quote expr env) (shape expr 2) (gen-cst (cadr expr)))

;; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

(define (comp-quasiquote expr env) (comp-quasiquotation (cadr expr) 1 env))
    ...
    ((pair? form)
      (cond
        ...
        ((eq? (car form) 'unquote-splicing)
          (when (= level 1) (scheme-error "Ill-placed 'unquote-splicing'" form))
          (comp-quasiquotation-list form (- level 1) env))
        (else (comp-quasiquotation-list form level env))))
    ((vector? form)
      (gen-vector-form (comp-quasiquotation-list (vector->lst form) level env)))
    (else (gen-cst form))))
...
(define (comp-unquote expr env) (scheme-error "Ill-placed 'unquote'" expr))
...
(define (parms->frame parms)
  (cond
    ((null? parms) '())
    ((pair? parms)
      (let ((x (car parms))) (variable x) (cons x (parms->frame (cdr parms)))))
    (else (variable parms) (list parms))))

(define (rest-param? parms)
  (cond ((pair? parms) (rest-param? (cdr parms))) ((null? parms) #f) (else #t)))
        ...
        (let ((expr (car body)))
          (cond
            ...
            (else
              (cond
                ((eq? (car expr) 'begin)
                  (letrec-defines vars vals (append (cdr expr) (cdr body)) env))
                ((eq? (car expr) 'define)
...
(define (definition-name expr)
  ...
  (let ((pattern (cadr expr)))
    (let ((name (if (pair? pattern) (car pattern) pattern)))
      (unless (symbol? name) (scheme-error "Identifier expected" name))
      name)))
...
(define (comp-cond expr env) (comp-cond-aux (cdr expr) env))
...
(define (comp-cond-aux clauses env)
  (if (pair? clauses)
      (let ((clause (car clauses)))
        (shape clause 1)
        (cond
          ((eq? (car clause) 'else) (shape clause 2) (comp-sequence (cdr clause) env))
          ((not (pair? (cdr clause)))
      ...
      ((symbol? x)
        ...
        (let ((y (caddr expr)))
          (let ((proc (cons 'lambda (cons (bindings->vars y) (cdddr expr)))))
            (scheme-comp (cons (list 'letrec (list (list x proc)) x) (bindings->vals y))
                         env))))
      ((pair? x)
...
(define (comp-begin expr env) (shape expr 2) (comp-sequence (cdr expr) env))
...
(define (comp-do expr env)
  ...
  (let ((bindings (cadr expr)) (exit (caddr expr)))
    ...
    (let* ((vars (bindings->vars bindings))
      ...
      (gen-letrec (list (gen-lambda (length vars)
                                            ...
                                            (gen-sequence (comp-sequence (cdddr expr) new-env2)
                                                          (gen-combination (gen-var-ref '(1 . 1))
                                                                           (comp-vals (bindings->steps bindings)
                                                                                      new-env2))))))
                  (gen-combination (gen-var-ref '(0 . 1))
...
(define (gen-lambda-0 body) (lambda (rte) (lambda () (body rte))))

(define (gen-lambda-1 body) (lambda (rte) (lambda (a) (body (vector rte a)))))
  ...
  (lambda (rte)
    (lambda (a b c . d)
      (let ((x (make-vector (+ nb-vars 1))))
        ...
        (vector-set! x 3 c)
        (let loop ((n nb-vars) (x x) (i 4) (l d))
          (when (<= i n) (vector-set! x i (car l)) (loop n x (+ i 1) (cdr l))))
        (body x)))))

;; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

(define (gen-sequence code1 code2) (lambda (rte) (code1 rte) (code2 rte)))
...
(define (gen-cond-send code1 code2 code3)
  (lambda (rte)
    (let ((temp (code1 rte))) (if temp ((code2 rte) temp) (code3 rte)))))
...
(define (gen-letrec-1 val1 body)
  (lambda (rte) (let ((x (vector rte #f))) (vector-set! x 1 (val1 x)) (body x))))
  ...
  (lambda (rte)
    (let ((x (make-vector (+ nb-vals 1))))
      ...
      (let loop ((x x) (i 1) (l vals))
        (when (pair? l) (vector-set! x i ((car l) x)) (loop x (+ i 1) (cdr l))))
      (body x))))
...
(define (gen-combination-1 oper arg1) (lambda (rte) ((oper rte) (arg1 rte))))
...
(define (scheme-global-var-set! i val) (set-cdr! i val) '())
  ...
  (let* ((count 100000)
         (input1
            '(let ()
               ...
               (sort-list '("one" "two" "three" "four" "five" "six" "seven" "eight" "nine" "ten"
         ...
         (output
            '("eight" "eighteen" "eleven" "fifteen" "five" "four" "fourteen" "nine" "nineteen" "one"
              "seven" "seventeen" "six" "sixteen" "ten" "thirteen" "thirty" "three" "twelve"
              "twenty" "twentyeight" "twentyfive" "twentyfour" "twentynine" "twentyone"
              "twentyseven" "twentysix" "twentythree" "twentytwo" "two"))
