...
(define (count z0 step z)
  ...
  (let* ((max-count 64) (radius 4.0) (radius^2 (* radius radius)))
    ...
    (let ((z0 (+ z0 (* z step))))
      ...
      (let loop ((z z0) (c 0))
        (if (= c max-count)
            c
            (let* ((zr (real-part z)) (zi (imag-part z)) (zr^2 (* zr zr)) (zi^2 (* zi zi)))
...
(define (test n)
  (let ((matrix (make-vector n)))
    (let loop ((i (- n 1)))
      (when (>= i 0) (vector-set! matrix i (make-vector n)) (loop (- i 1))))
