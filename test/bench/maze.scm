...
(define (random-int n state) (mod (rand state) n))
...
(define (get-set-root s)
  (let lp ((r s)) ;; Find the last pair
    (let ((next (cdr r))) ;; in the list. That's
      (cond
        ((pair? next) (lp next)) ;; the root r.

        (else
          (if (not (eq? r s)) ;; Now zip down the list again,
              (let lp ((x s)) ;; changing everyone's cdr to r.
                (let ((next (cdr x))) (cond ((not (eq? r next)) (set-cdr! x r) (lp next))))))
          r))))) ;; Then return r.
...
(define (union! s1 s2)
  (let* ((r1 (get-set-root s1))
    ...
    (cond
      ((> n1 n2) (set-cdr! r2 r1) (set-car! r1 n))
...
(define (make-wall owner neighbor bit) (vector 'wall owner neighbor bit))
...
(define (make-cell reachable id) (vector 'cell reachable id -1 #f #f))
...
(define (permute-vec! v random-state)
  (let lp ((i (- (vector-length v) 1)))
    (cond
      ((> i 1)
        (let ((elt-i (vector-ref v i)) (j (random-int i random-state))) ;; j in [0,i)
...
(define (dig-maze walls ncells)
  (call-with-current-continuation (lambda (quit)
                                    (vector-for-each-rev (lambda (wall) ;; For each wall,
...
(define (dfs-maze maze root do-children)
  (let search ((node root) (parent #f))
    ...
    (do-children (lambda (child) (if (not (eq? child parent)) (search child node)))
                 maze
                 node)))
...
(define (path-length cell)
  (do ((len 0 (+ len 1)) (node (cell:parent cell) (cell:parent node)))
       ((not node) len)))
...
(define (make-harr nrows ncols elts) (vector 'harr nrows ncols elts))
...
(define (harr r c) (make-harr r c (make-vector (* r c))))

(define (href ha x y)
  (let ((r (div y 2)) (c (div x 3)))
    (vector-ref (harr:elts ha) (+ (* (harr:ncols ha) r) c))))

(define (hset! ha x y val)
  (let ((r (div y 2)) (c (div x 3)))
    (vector-set! (harr:elts ha) (+ (* (harr:ncols ha) r) c) val)))
...
(define (harr-for-each proc harr) (vector-for-each-rev proc (harr:elts harr)))
...
(define (make-wall-vec harr)
  (let* ((nrows (harr:nrows harr))
    ...
    (do ((x (* (- ncols 1) 3) (- x 3)))
         ((< x 0))
         (do ((y (+ (* (- nrows 1) 2) (bitwise-and x 1)) (- y 2)))
              ((<= y 1)) ;; Don't do bottom row.
              (let ((hex (href harr x y)))
                (if (not (zero? x)) (add-wall hex (href harr (- x 3) (- y 1)) south-west))
                (add-wall hex (href harr x (- y 2)) south)
                (if (< x xmax) (add-wall hex (href harr (+ x 3) (- y 1)) south-east)))))
    ...
    (if (> ncols 1)
        (let ((rmoc-x (+ 3 (* 6 (div (- ncols 2) 2)))))
          ...
          (do ((x
                (- rmoc-x 6) ;; Do the rest of the bottom row's odd cols.
                (- x 6)))
...
(define (pick-entrances harr)
  ...
  (let ((nrows (harr:nrows harr)) (ncols (harr:ncols harr)))
    (let tp-lp ((max-len -1) (entrance #f) (exit #f) (tcol (- ncols 1)))
      (if (< tcol 0)
...
(define (make-maze nrows ncols)
  (let* ((cells (gen-maze-array nrows ncols))
    ...
    (let ((result (pick-entrances cells)))
      (let ((entrance (vector-ref result 0)) (exit (vector-ref result 1)))
        (let* ((exit-cell (href/rc cells 0 exit)) (walls (cell:walls exit-cell)))
...
(define (write-ch c) (set! output (cons c output)))
...
(define (print-hexmaze harr entrance)
  (let* ((nrows (harr:nrows harr))
    ...
    (do ((c 0 (+ c 2)))
         ...
         (write-ch #\\))
    (if (odd? ncols) (write-ch (if (= entrance (- ncols 1)) #\space #\_)))
    ;;   (newline)
    (write-ch #\newline)
    ...
    (do ((r (- nrows 1) (- r 1)))
         ...
         (cond ((odd? ncols) (write-ch (dot/space harr r (- ncols 1))) (write-ch #\\)))
         ;;     (newline)
         (write-ch #\newline)
         ...
         (cond
           ((odd? ncols) (display-hexbottom (cell:walls (href/rc harr r (- ncols 1)))))
           ((not (zero? r)) (write-ch #\\)))
         ;;     (newline)
         (write-ch #\newline))))

(define (bit-test j bit) (not (zero? (bitwise-and j bit))))
...
(define (main)
  (let* ((count 10000)
         ...
         (input2 7)
         (output
            '(#\space #\space #\space #\_ #\space #\space #\space #\_ #\space #\space #\space #\_
              #\newline #\space #\_ #\/ #\space #\\ #\_ #\/ #\space #\\ #\_ #\/ #\. #\\ #\space
              #\newline #\/ #\space #\\ #\space #\space #\space #\\ #\_ #\space #\. #\space #\space
              #\/ #\. #\\ #\newline #\\ #\space #\space #\space #\\ #\space #\/ #\. #\space #\_ #\/
              #\. #\\ #\space #\/ #\newline #\/ #\space #\\ #\_ #\/ #\. #\space #\_ #\/ #\space #\\
              #\_ #\space #\. #\\ #\newline #\\ #\space #\/ #\space #\\ #\space #\/ #\space #\space
              #\_ #\/ #\space #\\ #\_ #\/ #\newline #\/ #\space #\space #\_ #\/ #\. #\\ #\space #\/
              #\space #\\ #\space #\/ #\space #\\ #\newline #\\ #\space #\/ #\space #\\ #\space #\/
              #\space #\space #\_ #\/ #\space #\space #\space #\/ #\newline #\/ #\space #\\ #\space
              #\/ #\. #\\ #\space #\/ #\. #\\ #\_ #\/ #\space #\\ #\newline #\\ #\_ #\/ #\space #\\
              #\space #\/ #\. #\space #\_ #\space #\. #\\ #\space #\/ #\newline #\/ #\space #\\ #\_
              #\space #\. #\space #\_ #\/ #\space #\\ #\space #\space #\space #\\ #\newline #\\ #\_
              #\space #\space #\\ #\_ #\/ #\space #\space #\_ #\/ #\. #\\ #\space #\/ #\newline #\/
              #\space #\space #\_ #\/ #\space #\space #\space #\/ #\space #\\ #\space #\/ #\space
              #\\ #\newline #\\ #\_ #\space #\space #\\ #\space #\/ #\space #\\ #\_ #\space #\. #\\
              #\_ #\/ #\newline #\/ #\space #\\ #\_ #\space #\space #\\ #\_ #\space #\space #\\ #\_
              #\space #\. #\\ #\newline #\\ #\_ #\space #\space #\\ #\_ #\/ #\space #\space #\_ #\/
              #\. #\\ #\space #\/ #\newline #\/ #\space #\\ #\_ #\space #\space #\\ #\space #\/ #\.
