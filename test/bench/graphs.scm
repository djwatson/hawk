...
(define fold
  (lambda (lst folder state)
    ...
    ;            folder)
    (do ((lst lst (cdr lst)) (state state (folder (car lst) state)))
         ((null? lst) state))))
  ...
  (lambda (size f)
    ...
    (if (zero? size)
        ...
        (let ((x (make-vector size (f 0))))
          (let loop ((i 1))
            (if (< i size) (begin (vector-set! x i (f i)) (loop (+ i 1)))))
          x))))
...
(define vector-fold
  (lambda (vec folder state)
    ...
    (let ((len (vector-length vec)))
      (do ((i 0 (+ i 1)) (state state (folder (vector-ref vec i) state)))
           ((= i len) state)))))
...
(define natural-for-all?
  (lambda (limit ok?)
    ...
    ;        (assert (procedure? ok?)
    ;            ok?)
    (let _-*- ((i 0)) (or (= i limit) (and (ok? i) (_-*- (+ i 1)))))))
...
(define natural-there-exists?
  (lambda (limit ok?)
    ...
    ;        (assert (procedure? ok?)
    ;            ok?)
    (let _-*- ((i 0)) (and (not (= i limit)) (or (ok? i) (_-*- (+ i 1)))))))
...
(define fold-over-perm-tree
  (lambda (universe b-folder b-state t-folder t-state)
    ...
    (let _-*- ((universe universe)
      ...
      (if (null? universe)
          (t-folder b-state t-state accross)
          (let _-**- ((in universe) (out '()) (t-state t-state))
            (let* ((first (car in))
                   (rest (cdr in))
                   (accross
                      (if (null? rest)
...
(define make-minimal?
  (lambda (max-size)
    ...
    ;                (>= max-size 0))
    ;            max-size)
    (let ((iotas (proc->vector (+ max-size 1) giota)) (perm (make-vector max-size 0)))
        ...
        (fold-over-perm-tree (vector-ref iotas size)
                             (lambda (perm-x x state deeper accross)
                               (case (cmp-next-vertex graph perm x perm-x)
                                 ((less) #f)
                                 ((equal) (vector-set! perm x perm-x) (deeper (+ x 1) state))
                                 ((more) (accross state))
                                 (else
                                   ;                                (assert #f)
                                   (error "???"))))
...
(define cmp-next-vertex
  (lambda (graph perm x perm-x)
    (let ((from-x (vector-ref graph x)) (from-perm-x (vector-ref graph perm-x)))
      (let _-*- ((y 0))
        (if (= x y)
            'equal
            (let ((x->y? (vector-ref from-x y)) (perm-y (vector-ref perm y)))
              (cond
      ...
      (let _-*- ((vertex 0) (state state))
        (cond
          ...
          ((= vertex root)
            ...
            ;                                            (vector-ref edges v))))
            ;                                #t))
            (let ((reach? (make-reach? root edges)) (from-root (vector-ref edge? root)))
              (let _-*- ((v 0) (outs 0) (efr '()) (efrr '()) (state state))
                (cond
          ...
          (else
            (let ((from-vertex (vector-ref edge? vertex)))
              (let _-**- ((sv 0) (outs 0) (state state))
                (if (= sv vertex)
                    (begin (vector-set! out-degrees vertex outs) (_-*- (+ vertex 1) state))
                    (let* ((state
...
(define (run n) (fold-over-rdg n 2 cons '()))

;-----

(let ((r (run 5))) (pp (length r)) (pp r))
