...
(define (run-benchmark2 name thunk) (display name) (newline) (thunk))
...
(define (gcbench kStretchTreeDepth)
  ...
  (define (TreeSize i) (- (expt 2 (+ i 1)) 1))

  ;;  Number of iterations to use for a given tree depth
  (define (NumIters i)
    (quotient (* 2 (TreeSize kStretchTreeDepth)) (TreeSize i)))
      ...
      (define (TimeConstruction depth)
        (let ((iNumIters (NumIters depth)))
          (display (string-append "Creating "
                                  ...
                                  (number->string depth)))
          (newline)
          (run-benchmark2 "GCBench: Top down construction"
                          (lambda ()
                            (do ((i 0 (+ i 1)))
                                 ((>= i iNumIters))
                                 (Populate depth (make-empty-node)))))
          (run-benchmark2 "GCBench: Bottom up construction"
                          (lambda () (do ((i 0 (+ i 1))) ((>= i iNumIters)) (MakeTree depth))))))

      (define (main)
        (display "Garbage Collector Test")
        (newline)
        (display (string-append " Stretching memory with a binary tree of depth "
                                (number->string kStretchTreeDepth)))
        (newline)
        (PrintDiagnostics)
        (run-benchmark2 "GCBench: Main"
                        (lambda ()
                          ;;  Stretch the memory space quickly
                          (MakeTree kStretchTreeDepth)

                          ;;  Create a long lived object
                          (display (string-append " Creating a long-lived binary tree of depth "
                          ...
                          (let ((longLivedTree (make-empty-node)))
                            ...
                            (display (string-append " Creating a long-lived array of "
                            ...
                            (let ((array (make-vector kArraySize 0)))
                              ...
                              (if (or (eq? longLivedTree '())
                                     (let ((n (min 1000 (- (quotient (vector-length array) 2) 1))))
                                       (not (= (vector-ref array n) (/ 1 (+ n 1))))))
