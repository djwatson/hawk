...
(define (adjoin element set) (if (memq element set) set (cons element set)))
...
(define (has-op? op node) (not (null? (lookup-op op node))))
...
(define (copy-graph g)
  ...
  (make-internal-graph (copy-list (graph-nodes g))
                       (already-met g)
                       (already-joined g)))
...
(define (conforms? t1 t2)
  ...
  (define (delete-red-edges! from-node) (set-red-edges! from-node '()))
    ...
    (cond
      ...
      (else
        ...
        (let loop ((blues (blue-edges t2)))
          (if (null? blues)
              #t
              (let* ((current-edge (car blues)) (phi (operation current-edge)))
                (and (has-op? phi t1)
...
(define (equivalent? a b) (and (conforms? a b) (conforms? b a)))

;; EQUIVALENCE CLASSIFICATION
;; Given a list of nodes, return a list of equivalence classes

(define (classify nodes)
  (let node-loop ((classes '()) (nodes nodes))
    (if (null? nodes)
        (map (lambda (class)
               (sort-list class
                          (lambda (node1 node2)
...
(define (insert! table x y value)
  (define (make-singleton-table x y) (list (cons x y)))
  (let ((one (assq x (cdr table))))
    (if one
    ...
    (else
      (let ((result
               (make-node (string-append "("
                                         (internal-node-name node1)
                                         " ^ "
                                         (internal-node-name node2)
                                         ")"))))
...
(define (make-lattice g print?)
  (define (step g)
    (let* ((copy (copy-graph g)) (nodes (graph-nodes copy)))
      (for-each (lambda (first)
                  (for-each (lambda (second) (meet copy first second) (join copy first second))
                            nodes))
                nodes)
      copy))
  (define (loop g count)
    ...
    (let ((lattice (step g)))
      (when print? (display " -> ") (display (length (graph-nodes lattice))))
      (let* ((new-g (xreduce lattice)) (new-count (length (graph-nodes new-g))))
        (if (= new-count count)
            (begin (when print? (newline)) new-g)
            (begin
              (when print? (display " -> ") (display new-count) (newline))
...
(define (setup a0 b0 c0 d0)
  ...
  (set-blue-edges! b
                   (list (make-blue-edge 'phi any-node a) (make-blue-edge 'theta any-node b)))
  (set! c (make-node c0))
  (set! d (make-node d0))
  (set-blue-edges! c (list (make-blue-edge 'theta any-node b)))
  (set-blue-edges! d
                   (list (make-blue-edge 'phi any-node c) (make-blue-edge 'theta any-node d)))
  '(made a b c d))
...
(define (main)
  (let* ((count 500)
         (input1 '(a b "c" "d"))
         (output
            '("(((b v d) ^ a) v c)" "(c ^ d)" "(b v (a ^ d))" "((a v d) ^ b)" "(b v d)"
              "(b ^ (a v c))" "(a v (c ^ d))" "((b v d) ^ a)" "(c v (a v d))" "(a v c)"
              "(d v (b ^ (a v c)))" "(d ^ (a v c))" "((a ^ d) v c)" "((a ^ b) v d)"
              "(((a v d) ^ b) v (a ^ d))" "(b ^ d)" "(b v (a v d))" "(a ^ c)" "(b ^ (c v d))"
              "(a ^ b)" "(a v b)" "((a ^ d) ^ b)" "(a ^ d)" "(a v d)" "d" "(c v d)" "a" "b" "c"
              "any" "none"))
