...
(define (sumlists x)
  (do ((x x (cdr x))
       (sum 0 (do ((y (car x) (cdr y)) (sum sum (+ sum (car y)))) ((null? y) sum))))
       ((null? x) sum)))

(define (one..n n) (do ((n n (- n 1)) (p '() (cons n p))) ((zero? n) p)))
        ...
        (define (flush-queue)
          (let loop ((i 0))
            (when (< i k)
              (vector-set! queue
                           i
                           (let ((j (+ i ell))) (if (< j k) (vector-ref queue j) '())))
              (loop (+ i 1)))))

        (fill-queue 0 (- k ell))
        (run-benchmark id
                       m
                       (lambda () (fill-queue (- k ell) k) (flush-queue) queue)
                       (lambda (q)
                         (define (factorial n) (if (= n 0) 1 (* n (factorial (- n 1)))))
                         (let ((q0 (vector-ref q 0)) (qi (vector-ref q (max 0 (- k ell 1)))))
                           (and (or (and (null? q0) (null? qi))
                                   (and (pair? q0) (pair? qi) (equal? (car q0) (car qi))))
                                (= (sumlists q0) (/ (* n (+ n 1) (factorial n)) 2)))))))
      (begin (display "Incorrect arguments to MpermNKL-benchmark") (newline))))

(define (run-benchmark . args) (apply run-r7rs-benchmark args))
