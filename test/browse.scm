...
(define (lookup key table)
  (let loop ((x table))
    (if (null? x)
        #f
        (let ((pair (car x))) (if (eq? (car pair) key) pair (loop (cdr x)))))))

(define *properties* '())

(define (get key1 key2)
  (let ((x (lookup key1 *properties*)))
    (if x (let ((y (lookup key2 (cdr x)))) (if y (cdr y) #f)) #f)))

(define (put key1 key2 val)
  (let ((x (lookup key1 *properties*)))
    (if x
        (let ((y (lookup key2 (cdr x))))
          (if y (set-cdr! y val) (set-cdr! x (cons (cons key2 val) (cdr x)))))
...
(define (tree-copy x)
  (if (not (pair? x)) x (cons (tree-copy (car x)) (tree-copy (cdr x)))))
            ...
            ((eq? (string-ref (symbol->string (car pat)) 0) #\*)
              (let ((val (assq (car pat) alist)))
                (cond
                  ...
                  (else
                    (do ((l '() (append-to-tail! l (cons (if (null? d) '() (car d)) '())))
                         (e (cons '() dat) (cdr e))
...
(define (investigate units pats)
  (do ((units units (cdr units)))
       ...
       (do ((pats pats (cdr pats)))
            ((null? pats))
            (do ((p (get (car units) 'pattern) (cdr p)))
                 ((null? p))
...
(define (main)
  (let* ((count 2000)
         ...
         (output
            '(837 177 1090 617 661 749 628 56 826 408 1035 474 320 452 672 991 155 122 793 221 716
              727 848 309 144 936 100 881 287 430 23 771 232 804 958 650 1068 1057 463 276 1046 1002
              199 34 738 210 540 397 342 364 782 683 89 375 166 595 892 705 507 639 331 188 243 441
              1013 1079 67 298 386 573 859 133 760 12 529 815 111 496 45 265 925 903 254 78 551 606
              485 518 419 870 562 1 353 980 694 914 969 947 584 1024))
         (s2 (number->string count))
         (s1 "")
         (name "browse"))
    (run-r7rs-benchmark (string-append name ":" s2)
                        count
                        (lambda () (browse input1))
