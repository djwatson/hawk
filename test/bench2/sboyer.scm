...
(define (main)
  (let* ((count 1)
    ...
    (run-r7rs-benchmark (string-append name ":" s1 ":" s2)
                        count
                        (lambda () (setup-boyer) (test-boyer alist term (hide count input)))
                        (lambda (rewrites) (and (number? rewrites) (= rewrites output))))))

(define alist
  (quote ((x f (plus (plus a b) (plus c (zero))))
...
(let ()
  ...
  (define (setup)
    (add-lemma-lst (quote ((equal (compile form) (reverse (codegen (optimize form) (nil))))
  ...
  (define (add-lemma-lst lst)
    (cond ((null? lst) #t) (else (add-lemma (car lst)) (add-lemma-lst (cdr lst)))))
  ...
  (define (get sym property) (get-lemmas (symbol->symbol-record sym)))
  ...
  (define (make-symbol-record sym) (vector sym '()))

  (define (put-lemmas! symbol-record lemmas)
    (vector-set! symbol-record 1 lemmas))

  (define (get-lemmas symbol-record) (vector-ref symbol-record 1))

  (define (get-name symbol-record) (vector-ref symbol-record 0))
  ...
  (define (test alist term n)
    (let ((term
             (apply-subst (translate-alist alist)
  ...
  (define (apply-subst alist term)
    (cond
      ((not (pair? term))
        (let ((temp-temp (assq term alist))) (if temp-temp (cdr temp-temp) term)))
      ...
      ((not (pair? term2))
        (let ((temp-temp (assq term2 unify-subst)))
          (cond
            ...
            (else (set! unify-subst (cons (cons term2 term1) unify-subst)) #t))))
      ((not (pair? term1)) #f)
  ...
  (define (falsep x lst) (or (term-equal? x false-term) (term-member? x lst)))

  (define (truep x lst) (or (term-equal? x true-term) (term-member? x lst)))
