...
(define (minroot a b c)
  (if (zero? a)
      ...
      (let ((disc (- (sq b) (* 4.0 a c))))
        (if (negative? disc)
            #f
            (let ((discrt (sqrt disc)) (minus-b (- b)) (two-a (* 2.0 a)))
...
(define (sendray pt ray)
  (let* ((x (first-hit pt ray)) (s (vector-ref x 0)) (int (vector-ref x 1)))
    (if s (* (lambert s int ray) (surface-color s)) 0.0)))

(define (first-hit pt ray)
  (let loop ((lst *world*) (surface #f) (hit #f) (dist 1e+308))
    (if (null? lst)
        (vector surface hit)
        (let ((s (car lst)))
...
(define (surface-color s) (sphere-color s))

(define (intersect s pt ray) (sphere-intersect s pt ray))
...
(define (normal s pt) (sphere-normal s pt))
...
(define (run input output) (ray-test input output) 'ok)
