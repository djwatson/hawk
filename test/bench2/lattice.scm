  ...
  (lambda (test lst)
    (define select-a
      (lambda (ac lst)
        (if (null? lst)
            (xreverse! ac)
            (select-a (let ((head (car lst))) (if (test head) (cons head ac) ac))
                      (cdr lst)))))
    (select-a '() lst)))
  ...
  (lambda (test func lst)
    (define select-a
      (lambda (ac lst)
        (if (null? lst)
            (xreverse! ac)
            (select-a (let ((head (car lst))) (if (test head) (cons (func head) ac) ac))
                      (cdr lst)))))
    (select-a '() lst)))
...
(define (maps-1 source target pas new)
  (let ((scmp (lattice->cmp source)) (tcmp (lattice->cmp target)))
