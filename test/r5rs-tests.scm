...
(define-syntax test
   (syntax-rules ()
     ((test name expect expr) (test expect expr))
     ((test expect expr)
      (begin
        ...
        (let ((res expr))
          (flush-output-port)
          (if (equal? res expect)
              (begin (set! *tests-passed* (+ *tests-passed* 1)) (display " [PASS]\n"))
              (begin
                (display " [FAIL]\n")
                (display "    expected ")
                (write expect)
...
(define-syntax test-assert
   (syntax-rules () ((test-assert expr) (test #t expr))))
...
;; FIXED from source: these are required to be flonum
;; in r7rs if any input is flonum
(test 4.0 (max 3.9 4))
...
(test 'ok
      (let ((... 2))
        (let-syntax ((s (syntax-rules () ((_ x ...) 'bad) ((_ . r) 'ok)))) (s a b c))))

;; r7rs does not have splicing let-syntax and letrec-syntax
;; (test 'ok (let ()
;;             (let-syntax ()
...
(test 2
      (let-syntax ((foo (syntax-rules ::: () ((foo ... args :::) (args ::: ...)))))
        (foo 3 - 5)))
