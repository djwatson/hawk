    ...
    (if (and (> i 0) (not (char=? (string-ref x (- i 1)) file-path-sep)))
        ...
        (let ((result (make-string i)))
          (let loop2 ((j (- i 1)))
            (if (< j 0)
                result
                (begin (string-set! result j (string-ref x j)) (loop2 (- j 1)))))))))
(define (file-name x)
  (let loop1 ((i (string-length x)))
    (if (and (> i 0) (not (char=? (string-ref x (- i 1)) file-path-sep)))
        (loop1 (- i 1))
        (let ((result (make-string (- (string-length x) i))))
          (let loop2 ((j (- (string-length x) 1)))
            (if (< j i)
                result
                (begin (string-set! result (- j i) (string-ref x j)) (loop2 (- j 1)))))))))
(define (file-ext x)
  (let loop1 ((i (string-length x)))
...
(define (pos-in-list x l)
  (let loop ((l l) (i 0))
    (cond ((not (pair? l)) #f) ((eq? (car l) x) i) (else (loop (cdr l) (+ i 1))))))
...
(define (pair-up l1 l2)
  (define (pair l1 l2)
    (if (pair? l1) (cons (cons (car l1) (car l2)) (pair (cdr l1) (cdr l2))) '()))
  (pair l1 l2))
      ...
      (cond
        ...
        (else
          (let ((e1 (car l1)) (e2 (car l2)))
            (if (<? e1 e2) (cons e1 (merge (cdr l1) l2)) (cons e2 (merge l1 (cdr l2))))))))
    (define (split l)
...
(define (lst->string l)
  (let* ((n (length l)) (s (make-string n)))
    (let loop ((l l) (i 0))
      (if (pair? l) (begin (string-set! s i (car l)) (loop (cdr l) (+ i 1))) s))))
(define (string->lst s)
  ...
  (if (set-empty? graph)
      ...
      (let ((to-remove (or (remove-no-edges graph) (remove-cycle graph))))
        (let ((labels (set-map gnode-label to-remove)))
          (cons labels
                (topological-sort (set-map (lambda (x)
                                             (make-gnode (gnode-label x)
    ...
    (let ((edges (gnode-edges (car l))))
      ...
      (if (set-member? (gnode-label (car l)) edges)
          (let ((edge-graph (set-keep member-edges? graph)))
            (if (set-every? equal-edges? edge-graph) edge-graph (remove (cdr l))))
          (remove (cdr l)))))
...
(define (queue-put! queue x)
  (let ((entry (cons x '())))
    (if (null? (car queue)) (set-car! queue entry) (set-cdr! (cdr queue) entry))
    (set-cdr! queue entry)
    x))
...
(define (open-sf filename)
  ...
  (if (not (file-ext filename))
      (let loop ((exts scm-file-exts))
        (if (pair? exts)
            (let* ((ext (car exts))
                   (full-name (if ext (string-append filename "." ext) filename))
...
(define (sf-read-error sf msg . args)
  (apply compiler-user-error
         (cons (sf->locat sf) (cons (string-append "Read error -- " msg) args))))
(define (sf->locat sf)
  (vector 'file
          (vector-ref sf 1)
          (vector-ref sf 2)
...
(define (locat-show loc)
  (if loc
      (case (vector-ref loc 0)
        ((file)
          (if (pinpoint-error (vector-ref loc 1) (vector-ref loc 3) (vector-ref loc 4))
              (begin
                (display "file \"")
        ...
        ((expr)
          ...
          (if (vector-ref loc 2)
              (begin (display " ") (locat-show (source-locat (vector-ref loc 2))))))
...
(define (locat-filename loc)
  (if loc
      (case (vector-ref loc 0)
        ...
        (else (compiler-internal-error "locat-filename, unknown location tag")))
      ""))
...
(define (expression->source expr source)
  ...
  (define (list->source l)
    (cond
      ((pair? l) (cons (expr->source (car l)) (list->source (cdr l))))
      ((null? l) '())
      (else (expr->source l))))
  (define (vector->source v)
    (let* ((len (vector-length v)) (x (make-vector len)))
      (let loop ((i (- len 1)))
        (if (>= i 0)
            (begin (vector-set! x i (expr->source (vector-ref v i))) (loop (- i 1)))))
      x))
  (expr->source expr))
(define (source->expression source)
  (define (list->expression l)
    (cond
      ((pair? l) (cons (source->expression (car l)) (list->expression (cdr l))))
      ((null? l) '())
  ...
  (define (vector->expression v)
    (let* ((len (vector-length v)) (x (make-vector len)))
      (let loop ((i (- len 1)))
        (if (>= i 0)
            (begin (vector-set! x i (source->expression (vector-ref v i))) (loop (- i 1)))))
      x))
  (let ((code (source-code source)))
    (cond
      ((pair? code) (list->expression code))
    ...
    (define (read-sources)
      (let ((source (read-source sf)))
        (if (not (eof-object? source))
            (begin (if info-port (display "." info-port)) (cons source (read-sources)))
            '())))
...
(define (read-source sf)
  (define (read-char*)
    (let ((c (sf-read-char sf)))
      (if (eof-object? c) (sf-read-error sf "Premature end of file encountered") c)))
  (define (read-non-whitespace-char)
    (let ((c (read-char*)))
      (cond
        ((< 0 (vector-ref read-table (char->integer c))) (read-non-whitespace-char))
        ((char=? c #\;)
          (let loop ()
      ...
      (let loop ((end result))
        (let ((c (read-non-whitespace-char)))
          (cond
            ...
            (else (let ((tail (cons (rd* c) '()))) (set-cdr! end tail) (loop tail))))))
      result))
  (define (read-vector)
    (define (loop i)
      (let ((c (read-non-whitespace-char)))
  ...
  (define (read-string)
    (define (loop i)
      (let ((c (read-char*)))
        (cond
          ((char=? c #\") (make-string i #\space))
          ((char=? c #\\)
            (let* ((c (read-char*)) (s (loop (+ i 1)))) (string-set! s i c) s))
  ...
  (define (rd c)
    (cond
      ((eof-object? c) c)
      ((< 0 (vector-ref read-table (char->integer c))) (rd (sf-read-char sf)))
      ((char=? c #\;)
        (let loop ()
          (let ((c (sf-read-char sf)))
            (cond
                              ...
                              (cond
                                ...
                                (else
                                  (let ((num (read-prefixed-number c)))
                                    (or num (sf-read-error sf "Unknown '#' read macro" c)))))))
                          ((char=? c #\") (read-string))
                          ((char=? c #\')
                            (list (make-source quote-sym (sf->locat sf)) (read-source sf)))
                          ((char=? c #\`)
                            (list (make-source quasiquote-sym (sf->locat sf)) (read-source sf)))
                          ((char=? c #\,)
                            (if (char=? (sf-peek-char sf) #\@)
                                (let ((x (make-source unquote-splicing-sym (sf->locat sf))))
                                  (sf-read-char sf)
                                  (list x (read-source sf)))
                                (list (make-source unquote-sym (sf->locat sf)) (read-source sf))))
    ...
    (if (pair? decls)
        (let ((decl (car decls)))
          (if (eq? (car decl) namespace-sym)
              (let ((syms (cddr decl)))
                (if (or (null? syms) (memq name syms)) (cadr decl) (loop (cdr decls))))
              (loop (cdr decls))))
        #f)))
(define (env-lookup env name stop-at-first-frame? proc)
  ...
  (define (search* env name full?)
    (define (search-macros macros)
      (if (pair? macros)
          (let ((m (car macros)))
            (if (eq? (car m) name) (proc env name (cdr m)) (search-macros (cdr macros))))
          (search-vars (env-vars-ref env))))
    (define (search-vars vars)
      (if (pair? vars)
          (let ((v (car vars)))
            (if (eq? (var-name v) name) (proc env name v) (search-vars (cdr vars))))
  ...
  (env-lookup env
              ...
              (lambda (env name x)
                (if x
                    (if (var? x)
                        x
                        (compiler-internal-error "env-lookup-var, name is that of a macro" name))
              ...
              (lambda (env name x)
                (if x
                    (if (var? x)
                        ...
                        (compiler-internal-error "env-define-var, name is that of a macro" name))
                    (env-new-var! env name source)))))
(define (env-lookup-global-var env name)
  (let ((env* (env-global-env env)))
    (define (search-vars vars)
      (if (pair? vars)
          (let ((v (car vars))) (if (eq? (var-name v) name) v (search-vars (cdr vars))))
...
(define (env-lookup-macro env name)
  (env-lookup env name #f (lambda (env name x) (if (or (not x) (var? x)) #f x))))
...
(define (declaration-value name element default decls)
  (if (not decls)
      ...
      (let loop ((l (env-decls-ref decls)))
        (if (pair? l)
            (let ((d (car l)))
              (if (and (eq? (car d) name) (or (null? (cddr d)) (memq element (cddr d))))
                  (cadr d)
                  (loop (cdr l))))
          ...
          (let ((var (ref-var oper)))
            (if (global? var)
                (let ((proc (standard-procedure (var-name var) (node-decl oper))))
                  (if (and proc
                           (not (nb-args-conforms? (length args)
...
(define (free-variables node)
  (if (eq? (node-fv node) #t)
      (let ((x (apply set-union (map free-variables (node-children node)))))
        (node-fv-set! node
                      (cond
                        ((ref? node) (if (global? (ref-var node)) x (set-adjoin x (ref-var node))))
                        ((set? node) (if (global? (set-var node)) x (set-adjoin x (set-var node))))
...
(define **make-placeholder-sym
  (string->canonical-symbol "##MAKE-PLACEHOLDER"))
  ...
  (define (parse-prog program env lst proc)
    (if (null? program)
        ...
        (let ((source (car program)))
          (cond
            ((macro-expr? source env)
              (parse-prog (cons (macro-expand source env) (cdr program)) env lst proc))
            ((begin-defs-expr? source)
              (parse-prog (append (begin-defs-body source) (cdr program)) env lst proc))
            ((include-expr? source)
              (if *ptree-port* (display "  " *ptree-port*))
              (let ((x
                       (file->sources* (include-filename source) *ptree-port* (source-locat source))))
                (if *ptree-port* (newline *ptree-port*))
                (parse-prog (append x (cdr program)) env lst proc)))
            ((define-macro-expr? source env)
              (if *ptree-port*
                  (begin (display "  \"macro\"" *ptree-port*) (newline *ptree-port*)))
              (parse-prog (cdr program) (add-macro source env) lst proc))
            ((declare-expr? source)
              (if *ptree-port*
                  (begin (display "  \"decl\"" *ptree-port*) (newline *ptree-port*)))
              (parse-prog (cdr program) (add-declarations source env) lst proc))
...
(define (c-interface-end)
  (let ((i
           (make-c-intf (reverse c-interface-decls)
  ...
  (and (mymatch **c-declaration-sym 1 source)
       (let ((code (source-code source)))
         (or (string? (source-code (cadr code)))
            (pt-syntax-error source "Argument to '##c-declaration' must be a string")))))
(define (c-init-expr? source)
  (and (mymatch **c-init-sym 1 source)
       (let ((code (source-code source)))
         (or (string? (source-code (cadr code)))
            (pt-syntax-error source "Argument to '##c-init' must be a string")))))
(define (c-procedure-expr? source)
  (and (mymatch **c-procedure-sym 3 source)
       (let ((code (source-code source)))
         (if (not (string? (source-code (cadddr code))))
             (pt-syntax-error source "Last argument to '##c-procedure' must be a string")
    ...
    (let ((res-type (convert-type res-typ)))
      (if (not res-type)
          ...
          (if (not (proper-length arg-typs))
              (pt-syntax-error arg-typs-source "Ill-terminated argument type list")
              (let loop ((lst arg-typs))
                (if (pair? lst)
                    (let* ((arg-typ (source-code (car lst))) (arg-type (convert-type arg-typ)))
...
(define (pt-c-procedure source env use)
  (let* ((code (source-code source))
         (name
            (build-c-procedure (map source-code (source-code (cadr code)))
...
(define (build-c-procedure argument-types result-type proc-name-or-code)
  ...
  (define (listify strings)
    (if (null? strings)
        ...
        (string-append (car strings)
                       (apply string-append (map (lambda (s) (string-append "," s)) (cdr strings))))))
  ...
  (define (make-c-procedure arg-types res-type)
    (define (make-arg-decl)
      (apply string-append
             (map (lambda (t) (string-append (c-type-decl (car t)) " " (c-arg-var t) ";" nl))
                  arg-types)))
    ...
    (let* ((index (number->string c-interface-proc-count))
           ...
           (def
              (string-append (if (or proc-name? (eq? res-type void-sym))
                                 ""
                                 (string-append (c-type-decl res-type)
                                                " "
                                                c-id-prefix
                                                c-result-name
                                                ";"
  ...
  (let loop ((i (- (string-length s) 1)) (l '()))
    (if (>= i 0)
        (let ((c (string-ref s i)))
          (cond
            ((or (char-alphabetic? c) (char-numeric? c)) (loop (- i 1) (cons c l)))
...
(define (pt source env use)
  (cond
    ((macro-expr? source env) (pt (macro-expand source env) env use))
    ...
    ((unquote-expr? source) (pt-syntax-error source "Ill-placed 'unquote'"))
    ...
    ((define-expr? source env) (pt-syntax-error source "Ill-placed 'define'"))
    ...
    ((declare-expr? source) (pt-syntax-error source "Ill-placed '##declare'"))
    ((c-declaration-expr? source)
      (pt-syntax-error source "Ill-placed '##c-declaration'"))
    ((c-init-expr? source) (pt-syntax-error source "Ill-placed '##c-init'"))
...
(define (macro-expand source env)
  (let ((code (source-code source)))
    (expression->source (apply (cdr (env-lookup-macro env (source-code (car code))))
  ...
  (cond
    ...
    ((vector? (source-code form))
      (vector-form form
                   ...
                   env))
    (else (new-cst form (env-declarations env) (source->expression form)))))
  ...
  (let ((code (source-code source)))
    (define (new-params parms)
      (cond
        ...
        ((null? parms) '())
        (else (list (make-var (source-code parms) #t (set-empty) (set-empty) parms)))))
    (define (min-params parms)
      (let loop ((l parms) (n 0))
        (if (pair? l) (if (pair? (source-code (car l))) n (loop (cdr l) (+ n 1))) n)))
                ...
                (let* ((var (car parm))
                  ...
                  (new-call* parm*
                             ...
                             (list (new-tst parm*
                                            ...
                                            (new-call* parm*
                                                       decl
                                                       (new-ref-extended-bindings parm*
                                                                                  **unassigned?-sym
                                                                                  env)
                                                       (list (new-ref parm*
                                                                      decl
    ...
    (if (eq? use 'none)
        ...
        (let* ((parms (source->parms (cadr code))) (frame (new-params parms)))
          (new-prc source
                   ...
                   (rest-param? parms)
                   frame
                   (optionals parms source (cddr code) (env-frame env frame)))))))
...
(define (pt-body source body env use)
  (define (letrec-defines vars vals envs body env)
    (cond
      ((null? body)
        (pt-syntax-error source "Body must contain at least one evaluable expression"))
      ...
      ((include-expr? (car body))
        (if *ptree-port* (display "  " *ptree-port*))
        (let ((x
                 (file->sources* (include-filename (car body))
                                 *ptree-port*
      ...
      ((declare-expr? (car body))
        (letrec-defines vars vals envs (cdr body) (add-declarations (car body) env)))
      ((define-macro-expr? (car body) env)
        (letrec-defines vars vals envs (cdr body) (add-macro (car body) env)))
      ...
      (else
        (let ((vars* (reverse vars)))
          (let loop ((vals* '()) (l1 vals) (l2 envs))
            (if (not (null? l1))
                (loop (cons (pt (car l1) (car l2) 'true) vals*) (cdr l1) (cdr l2))
  ...
  (define (pt-clauses clauses)
    (if (length? clauses 0)
        ...
        (let* ((clause* (car clauses)) (clause (source-code clause*)))
          (cond
            ((eq? (source-code (car clause)) else-sym)
...
(define (pt-and source env use)
  (define (pt-exprs exprs)
    (cond
      ((length? exprs 0) (new-cst source (env-declarations env) #t))
...
(define (pt-or source env use)
  (define (pt-exprs exprs)
    (cond
      ((length? exprs 0) (new-cst source (env-declarations env) false-object))
      ((length? exprs 1) (pt (car exprs) env use))
      (else
        (new-disj (car exprs)
                  (env-declarations env)
...
(define (pt-case source env use)
  (let ((code (source-code source)) (temp (new-temps source '(temp))))
            ...
            (if (eq? (source-code (car clause)) else-sym)
                ...
                (new-tst clause*
                         ...
                         (new-call* clause*
                                    ...
                                    (list (new-ref clause* (env-declarations env) (car temp))
                                          (new-cst (car clause)
                                                   (env-declarations env)
    ...
    (new-call* source
               (env-declarations env)
               (new-prc source (env-declarations env) #f 1 #f temp (pt-clauses (cddr code)))
      ...
      (if (null? vars*)
          ...
          (let ((var (car vars*)) (val (car vals*)))
            (set-adjoin (dgraph (cdr vars*) (cdr vals*))
                        (make-gnode var (set-intersection (list->set vars) (free-variables val)))))))
    (dgraph vars vals))
  (define (val-of var)
          ...
          (let loop1 ((l (reverse vars)) (vars-b '()) (vals-b '()) (vars-a '()))
            (if (not (null? l))
                (let* ((var (car l)) (val (val-of var)))
                  (if (or (prc? val) (set-empty? (set-intersection (free-variables val) vars-set)))
                      (loop1 (cdr l) (cons var vars-b) (cons val vals-b) vars-a)
                ...
                (let* ((result1
                  ...
                  result3))))))
  (set-prc-names! vars vals)
  (bind-in-order (topological-sort (transitive-closure (dependency-graph vars vals)))))
...
(define (pt-do source env use)
  (let* ((code (source-code source))
         ...
         (env (env-frame env vars))
         (step
            (map (lambda (x) (pt (if (length? x 2) (car x) (caddr x)) env 'true))
                 bindings))
...
(define (pt-delay source env use)
  (let ((code (source-code source)))
    (new-call* source
               ...
               (list (new-prc source (env-declarations env) #f 0 #f '() (pt (cadr code) env 'true))))))
...
(define (not-keyword source env name)
  (if (or (memq name common-keywords)
         (memq name
               (dialect-specific-keywords (scheme-dialect (env-declarations env)))))
      (pt-syntax-error source
                       "Predefined keyword can't be used as a variable:"
                       name)
      #t))
(define (set!-expr? source env)
  (and (mymatch set!-sym 2 source) (var-expr? (cadr (source-code source)) env)))
       ...
       (let ((code (source-code source)))
         (if (bindable-var? (cadr code) env)
             (and (proper-bindings? (caddr code) #t env)
                  (or (> (length code) 3) (pt-syntax-error source "Ill-formed named 'let'")))
...
(define (define-expr? source env)
  (and (mymatch define-sym -1 source)
       ...
       (let ((v (definition-variable source))) (not-macro v env (source-code v)))))
    ...
    (cond
      ...
      ((pair? pattern)
        ...
        (if (bindable-var? (car pattern) env)
            ...
            (pt-syntax-error (car pattern) "Procedure name must be an identifier")))
...
(define (definition-value def)
  (let ((code (source-code def)) (loc (source-locat def)))
    (cond
      ((pair? (source-code (cadr code)))
        (make-source (cons (make-source lambda-sym loc)
                           (cons (parms->source (cdr (source-code (cadr code))) loc) (cddr code)))
                     loc))
      ((null? (cddr code))
        (make-source (list (make-source quote-sym loc) (make-source undef-object loc))
                     loc))
      (else (caddr code)))))
                        ...
                        (let ((var (car parm)))
                          (if (bindable-var? var env)
                              (if (memq (source-code var) seen)
                                  (pt-syntax-error var "Duplicate parameter in parameter list")
                                  (proper-parms (cdr parms) (cons (source-code var) seen) #t))
                              (pt-syntax-error var "Parameter must be an identifier")))
                        (pt-syntax-error parm* "Ill-formed optional parameter")))
                  (pt-syntax-error parm* "optional parameters illegal in this dialect")))
            (optional-seen (pt-syntax-error parm* "Optional parameter expected"))
            ((bindable-var? parm* env)
              (if (memq parm seen)
                  (pt-syntax-error parm* "Duplicate parameter in parameter list"))
              (proper-parms (cdr parms) (cons parm seen) #f))
            (else (pt-syntax-error parm* "Parameter must be an identifier")))))
      ((null? parms) #t)
      ((bindable-var? parms env)
        (if (memq (source-code parms) seen)
            (pt-syntax-error parms "Duplicate parameter in parameter list")
            #t))
      (else (pt-syntax-error parms "Rest parameter must be an identifier"))))
       ...
       (let* ((clause* (car clauses))
         ...
         (if length
             (if (>= length 1)
                 (if (eq? (source-code (car clause)) else-sym)
                     (cond
                       ((= length 1) (pt-syntax-error clause* "Else clause must have a body"))
                       ((not (null? (cdr clauses)))
                         (pt-syntax-error clause* "Else clause must be the last clause"))
                       (else (proper-clauses (cdr clauses))))
                     (if (and (>= length 2)
                              (eq? (source-code (cadr clause)) =>-sym)
                              (not (= length 3)))
                         (pt-syntax-error (cadr clause)
...
(define (proper-selector-list? source)
  (let* ((code (source-code source)) (length (proper-length code)))
    (if length
        (or (>= length 1)
           (pt-syntax-error source "Selector list must contain at least one element"))
              ...
              (let ((var (car binding)))
                (if (bindable-var? var env)
                    (if (and check-dupl? (memq (source-code var) seen))
                        ...
                        (proper-bindings (cdr l) (cons (source-code var) seen)))
                    (pt-syntax-error var "Binding variable must be an identifier")))
        ...
        (let* ((pos (not (eq? (source-code (car code)) not-sym)))
          ...
          (if (not (pair? x))
              ...
              (let* ((id* (car x)) (id (source-code id*)))
                (cond
                  ((not (symbol-object? id))
                    (pt-syntax-error id* "Declaration name must be an identifier"))
                  ((assq id flag-declarations)
                    (cond
                      ((not pos) (pt-syntax-error id* "Declaration can't be negated"))
                      ((null? (cdr x)) (flag-decl source (cdr (assq id flag-declarations)) id))
                      (else (pt-syntax-error source "Ill-formed declaration"))))
                  ((memq id parameterized-declarations)
                    (cond
                      ((not pos) (pt-syntax-error id* "Declaration can't be negated"))
                      ((eqv? (proper-length x) 2)
                        (parameterized-decl source id (source->expression (cadr x))))
                      (else (pt-syntax-error source "Ill-formed declaration"))))
                  ...
                  ((memq id namable-boolean-declarations)
                    (namable-boolean-decl source id pos (map source->expression (cdr x))))
                  ((memq id namable-string-declarations)
                    (if (not (pair? (cdr x)))
                        ...
                        (let* ((str* (cadr x)) (str (source-code str*)))
                          (cond
                            ((not pos) (pt-syntax-error id* "Declaration can't be negated"))
                            ((not (string? str)) (pt-syntax-error str* "String expected"))
                            (else
                              (namable-string-decl source id str (map source->expression (cddr x))))))))
...
(define (normalize-parse-tree ptree env)
  (define (normalize ptree)
    (let ((tree (assignment-convert (partial-evaluate ptree) env)))
      ...
      tree))
  (if (def? ptree)
      (begin (node-children-set! ptree (list (normalize (def-val ptree)))) ptree)
      (normalize ptree)))
(define (partial-evaluate ptree) (pe ptree '()))
(define (pe ptree consts)
  (cond
    ((cst? ptree) (new-cst (node-source ptree) (node-decl ptree) (cst-val ptree)))
    ((ref? ptree)
    ...
    ((fut? ptree)
      (new-fut (node-source ptree) (node-decl ptree) (pe (fut-val ptree) consts)))
...
(define (pe-let ptree consts)
  (let* ((proc (app-oper ptree))
    ...
    (for-each (lambda (var) (var-refs-set! var (set-empty)) (var-sets-set! var (set-empty)))
              vars)
    (let loop ((l vars) (v vals) (new-vars '()) (new-vals '()) (new-consts consts))
      (if (null? l)
          (if (null? new-vars)
          ...
          (let ((var (car l)) (val (pe (car v) consts)))
            (if (and (set-member? var non-mut-vars) (cst? val))
                ...
                (loop (cdr l) (cdr v) (cons var new-vars) (cons val new-vals) new-consts)))))))
...
(define (ac ptree env mut)
  (cond
      ...
      (let ((var (set-var ptree))
        ...
        (if (global? var)
            ...
            (new-call source
                      ...
                      (list (new-ref source (node-decl ptree) (cdr (assq var mut))) val)))))
    ((tst? ptree)
      (new-tst (node-source ptree)
               (node-decl ptree)
    ...
    ((fut? ptree)
      (new-fut (node-source ptree) (node-decl ptree) (ac (fut-val ptree) env mut)))
             ...
             (if (null? mut-parms)
                 ...
                 (new-call (node-source ptree)
                           ...
                           (new-prc (node-source ptree)
                                    ...
                                    new-body)
                           (map (lambda (var)
                                  (new-call (var-source var)
                                            (node-decl ptree)
                                            (new-ref-extended-bindings (var-source var)
                                                                       **make-cell-sym
                                                                       env)
      ...
      (if (null? l)
          ...
          (let ((var (car l)) (val (car v)))
            (if (memq var mut-parms)
                (let ((src (node-source val)) (decl (node-decl val)) (var* (cdr (assq var mut))))
                      ...
                      (loop (cdr l)
                            ...
                            (new-seq src
                                     ...
                                     (new-call src
                                               ...
                                               (list (new-ref src decl var*) (ac val env mut)))
                                     new-body))
                      (loop (cdr l)
                            (cdr v)
                            (cons var* new-vars)
...
(define (ll! ptree cst-procs env)
  ...
  (cond
    ((or (cst? ptree)
        ...
        (disj? ptree)
        (fut? ptree))
      (for-each (lambda (child) (ll! child cst-procs env)) (node-children ptree)))
    ((prc? ptree)
...
(define (ll!-let ptree cst-procs env)
  (let* ((proc (app-oper ptree))
      ...
      (let loop ((cst-proc-vars
        ...
        (let* ((non-cst-proc-vars
               ...
               (cst-proc-vars*
                  (set-keep (lambda (var)
                              (let ((val (var->val var)))
                                (set-empty? (set-intersection (free-variables val)
                                                              non-cst-proc-vars))))
                            cst-proc-vars)))
    ...
    (define (transitively-closed-free-variables vars)
      (let ((tcfv-map
               (map (lambda (var) (cons var (free-variables (var->val var)))) vars)))
        (let loop ((changed? #f))
    ...
    (let* ((tcfv-map (transitively-closed-free-variables (liftable-proc-vars vars)))
...
(define (se ptree env num)
  (cond
    ((cst? ptree) (list quote-sym (cst-val ptree)))
    ((ref? ptree)
    ...
    ((conj? ptree)
      (list and-sym (se (conj-pre ptree) env num) (se (conj-alt ptree) env num)))
    ((disj? ptree)
      (list or-sym (se (disj-pre ptree) env num) (se (disj-alt ptree) env num)))
    ((prc? ptree)
      (let ((new-env (se-rename (prc-parms ptree) env num)))
        (list lambda-sym
              (se-parameters (prc-parms ptree) (prc-rest ptree) (prc-min ptree) new-env)
      ...
      (let ((oper (app-oper ptree)) (args (app-args ptree)))
        (if (and (prc? oper)
            ...
            (let ((new-env (se-rename (prc-parms oper) env num)))
              (list (if (set-empty? (set-intersection (list->set (prc-parms oper))
    ...
    (cond
      ...
      (else
        (let ((parm (cdr (assq (car parms) env))))
          (cons (if (> n 0) parm (list parm)) (se-parms (cdr parms) rest (- n 1) env))))))
...
(define (se-rename vars env num)
  (define (rename vars)
    (if (null? vars)
        env
        (cons (cons (car vars)
                    (string->canonical-symbol (string-append (symbol->string (var-name (car vars)))
                                                             "#"
    ...
    (if (< i *opnd-table-alloc*)
        ...
        (if (< *opnd-table-alloc* opnd-table-size)
            (begin
              ...
              i)
            (compiler-limitation-error "program is too long [virtual machine operand table overflow]")))))
...
(define (frame-first-empty-slot frame)
  (let loop ((i 1) (s (reverse (frame-slots frame))))
    (if (pair? s) (if (frame-live? (car s) frame) (loop (+ i 1) (cdr s)) i) i)))
(define (make-proc-obj name primitive? code call-pat side-effects? strict-pat type)
  (let ((proc-obj
           (vector proc-obj-tag
                   name
                   primitive?
...
(define (proc-obj? x)
  (and (vector? x) (> (vector-length x) 0) (eq? (vector-ref x 0) proc-obj-tag)))
...
(define (pattern-member? n pat)
  (cond
    ((pair? pat) (if (= (car pat) n) #t (pattern-member? n (cdr pat))))
    ((null? pat) #f)
    (else (<= pat n))))
...
(define (lbl-num->bb lbl-num bbs)
  (let loop ((bb-list (queue->list (bbs-bb-queue bbs))))
    (if (null? bb-list)
        (compiler-internal-error "lbl-num->bb missing label")
...
(define (bb-last-non-branch-instr bb)
  (let ((non-branch-instrs (bb-non-branch-instrs bb)))
    (if (null? non-branch-instrs)
        (bb-label-instr bb)
        (let loop ((l non-branch-instrs)) (if (pair? (cdr l)) (loop (cdr l)) (car l))))))
    ...
    (if (memq lbl-num seen)
        ...
        (let ((bb (lbl-num->bb lbl-num bbs)))
          (if (and (empty-bb? bb) (<= (bb-slots-gained bb) 0))
              (let ((jump-lbl-num (jump-to-non-entry-lbl? (bb-branch-instr bb))))
                (if jump-lbl-num
                    (jump-cascade-to jump-lbl-num
                                     ...
                                     (lambda (lbl-num fs poll?)
                                       (let* ((dest-bb (lbl-num->bb lbl-num bbs))
                                         ...
                                         (if (and (empty-bb? dest-bb)
                                                  (or (not poll?)
                                                     put-poll-on-ifjump?
                                                     (not (eq? (gvm-instr-type last-branch) 'ifjump))))
                                             (let* ((new-fs (+ fs (bb-slots-gained dest-bb)))
                                                    (new-frame
                                                       (frame-truncate (gvm-instr-frame branch)
                                                                       new-fs)))
                                               (define (adjust-opnd opnd)
                                                 (cond
                                                   ((stk? opnd)
                                                     (make-stk (+ (- fs
                                                                     (bb-entry-frame-size dest-bb))
                                                                  (stk-num opnd))))
                                                   ((clo? opnd)
                                                 ...
                                                 ((jump)
                                                   (bb-put-branch! bb
                                                                   (make-jump (adjust-opnd (jump-opnd last-branch))
                                                                              ...
                                                                              (gvm-instr-comment last-branch))))
                                                 (else
                                                   (compiler-internal-error "bbs-remove-jump-cascades!, unknown branch type"))))
                                             (bb-put-branch! bb
  ...
  (let ((new-bb-queue (queue-empty)) (scan-queue (queue-empty)))
    ...
    (define (scan-instr gvm-instr bb)
      (define (scan-opnd gvm-opnd)
        (cond
          ((lbl? gvm-opnd) (reachable (lbl-num->bb (lbl-num gvm-opnd) bbs) bb))
      ...
      (case (gvm-instr-type gvm-instr)
        ...
        ((copy) (scan-opnd (copy-opnd gvm-instr)) (scan-opnd (copy-loc gvm-instr)))
        ((close)
        ...
        (else
          (compiler-internal-error "bbs-remove-dead-code!, unknown GVM instruction type"))))
  ...
  (let* ((bb-list (queue->list (bbs-bb-queue bbs)))
    ...
    (define (hash-prim prim)
      (let ((n (length prim-table)) (i (pos-in-list prim prim-table)))
        (if i (- n i) (begin (set! prim-table (cons prim prim-table)) (+ n 1)))))
    (define (hash-opnds l)
      (let loop ((l l) (n 0))
        (if (pair? l)
            (loop (cdr l)
    ...
    (define (fix-map! bb1 bb2)
      (let loop ((l block-map))
        (if (pair? l)
            (let ((x (car l))) (if (= bb1 (cdr x)) (set-cdr! x bb2)) (loop (cdr l))))))
    (define (enter-bb! bb)
      (let ((h (hash-bb bb)))
    ...
    (define (add-bb bb l)
      (if (pair? l)
          (let ((bb* (car l)))
            (set! block-map (cons (cons (bb-lbl-num bb) (bb-lbl-num bb*)) block-map))
            (if (eqv-bb? bb bb*)
                (begin (fix-map! (bb-lbl-num bb) (bb-lbl-num bb*)) (set! changed? #t) l)
                (begin
                  (set! block-map (cdr block-map))
                  (if (eqv-gvm-instr? (bb-branch-instr bb) (bb-branch-instr bb*))
                      (extract-common-tail bb
                                           bb*
                                           (lambda (head head* tail)
                                             (if (null? tail)
                                                 ...
                                                 (let* ((lbl (bbs-new-lbl! bbs))
                                                        ...
                                                        (frame
                                                           (frame-truncate (gvm-instr-frame (if (null? head)
                                                                                                (bb-label-instr bb)
                                                                                                (car head)))
                                                                           fs**))
                                                        (bb**
                                                           (make-bb (make-label-simple lbl frame #f)
                                                                    bbs)))
    ...
    (define (eqv-list? pred? l1 l2)
      (if (pair? l1)
          (and (pair? l2) (pred? (car l1) (car l2)) (eqv-list? pred? (cdr l1) (cdr l2)))
          (not (pair? l2))))
    ...
    (define (eqv-gvm-opnd? opnd1 opnd2)
      (if (not opnd1)
          ...
          (and opnd2
               (cond
                 ((lbl? opnd1)
                   (and (lbl? opnd2) (eqv-lbl-num? (lbl-num opnd1) (lbl-num opnd2))))
                 ((clo? opnd1)
                   (and (clo? opnd2)
    ...
    (define (eqv-gvm-instr? instr1 instr2)
      (define (eqv-closure-parms? p1 p2)
        (and (eqv-gvm-opnd? (closure-parms-loc p1) (closure-parms-loc p2))
             ...
             (eqv-list? eqv-gvm-opnd? (closure-parms-opnds p1) (closure-parms-opnds p2))))
      (let ((type1 (gvm-instr-type instr1)) (type2 (gvm-instr-type instr2)))
        (and (eq? type1 type2)
             (frame-eq? (gvm-instr-frame instr1) (gvm-instr-frame instr2))
             (case type1
               ((label)
                 (let ((ltype1 (label-type instr1)) (ltype2 (label-type instr2)))
                   (and (eq? ltype1 ltype2)
                        (case ltype1
                          ((simple return task-entry task-return) #t)
                          ((entry)
                            (and (= (label-entry-min instr1) (label-entry-min instr2))
                                 (= (label-entry-nb-parms instr1) (label-entry-nb-parms instr2))
                                 (eq? (label-entry-rest? instr1) (label-entry-rest? instr2))
                                 (eq? (label-entry-closed? instr1) (label-entry-closed? instr2))))
                          (else (compiler-internal-error "eqv-gvm-instr?, unknown label type"))))))
               ((apply)
                 (and (eq? (apply-prim instr1) (apply-prim instr2))
                      (eqv-list? eqv-gvm-opnd? (apply-opnds instr1) (apply-opnds instr2))
               ...
               ((close)
                 (eqv-list? eqv-closure-parms? (close-parms instr1) (close-parms instr2)))
               ((ifjump)
                 (and (eq? (ifjump-test instr1) (ifjump-test instr2))
                      (eqv-list? eqv-gvm-opnd? (ifjump-opnds instr1) (ifjump-opnds instr2))
               ...
               (else (compiler-internal-error "eqv-gvm-instr?, unknown 'gvm-instr':" instr1))))))
...
(define (replace-label-references! bb replacement-lbl-num)
  (define (update-gvm-opnd opnd)
    (if opnd
        (cond
          ((lbl? opnd) (make-lbl (replacement-lbl-num (lbl-num opnd))))
          ((clo? opnd) (make-clo (update-gvm-opnd (clo-base opnd)) (clo-index opnd)))
          (else opnd))
        opnd))
  (define (update-gvm-instr instr)
    (define (update-closure-parms p)
      (make-closure-parms (update-gvm-opnd (closure-parms-loc p))
    ...
    (case (gvm-instr-type instr)
      ...
      ((copy)
        (make-copy (update-gvm-opnd (copy-opnd instr))
                   ...
                   (gvm-instr-comment instr)))
      ((close)
        (make-close (map update-closure-parms (close-parms instr))
                    (gvm-instr-frame instr)
                    (gvm-instr-comment instr)))
      ((ifjump)
      ...
      ((jump)
        (make-jump (update-gvm-opnd (jump-opnd instr))
                   ...
                   (gvm-instr-frame instr)
                   (gvm-instr-comment instr)))
      (else (compiler-internal-error "update-gvm-instr, unknown 'instr':" instr))))
  (bb-non-branch-instrs-set! bb
...
(define (bbs-order! bbs)
  (let ((new-bb-queue (queue-empty))
      ...
      (let loop ((l (bb-precedents bb)) (best #f) (best-fs #f))
        (if (null? l)
            ...
            (let* ((x (car l)) (x-fs (bb-exit-frame-size x)))
              (if (and (memq x left-to-schedule) (or (not best) (< x-fs best-fs)))
                  (loop (cdr l) x x-fs)
      ...
      (define (best-succ bb1 bb2)
        (if (branches-to-lbl? bb1)
            ...
            (if (branches-to-lbl? bb2)
                bb2
                (if (< (bb-exit-frame-size bb1) (bb-exit-frame-size bb2)) bb2 bb1))))
      (let ((branch (bb-branch-instr bb)))
        (case (gvm-instr-type branch)
          ((ifjump)
      ...
      (let ((x (succ-bb bb)))
        (if x
            (begin
              ...
              (let ((y (succ-bb bb))) (if y (schedule-around (remove-bb! y)))))))
      (schedule-refs bb))
    ...
    (define (schedule-refs bb)
      (for-each (lambda (x) (if (memq x left-to-schedule) (schedule-around (remove-bb! x))))
                (bb-references bb)))
...
(define (bbs->code-list bbs)
  (let ((code-list (linearize bbs))) (setup-slots-needed! code-list) code-list))
        ...
        (case (gvm-instr-type gvm-instr)
          ((label)
            (if (> sn-rest (frame-size (gvm-instr-frame gvm-instr)))
                (compiler-internal-error "setup-slots-needed!, incoherent slots needed for LABEL"))
...
(define (need-gvm-instrs non-branch branch)
  (if (pair? non-branch)
      (need-gvm-instr (car non-branch) (need-gvm-instrs (cdr non-branch) branch))
...
(define (need-gvm-opnd gvm-opnd slots-needed)
  (cond
    ((stk? gvm-opnd) (max (stk-num gvm-opnd) slots-needed))
    ((clo? gvm-opnd) (need-gvm-opnd (clo-base gvm-opnd) slots-needed))
    (else slots-needed)))
(define (need-gvm-opnds gvm-opnds slots-needed)
  (if (null? gvm-opnds)
      slots-needed
      (need-gvm-opnd (car gvm-opnds) (need-gvm-opnds (cdr gvm-opnds) slots-needed))))
        ...
        ((obj? gvm-opnd)
          (let ((val (obj-val gvm-opnd)))
            (if (and (proc-obj? val)
                ...
                (begin (queue-put! proc-seen val) (queue-put! proc-left val)))))
        ((clo? gvm-opnd) (scan-opnd (clo-base gvm-opnd)))))
    (define (dump-proc p)
      ...
      (display "] =" port)
      (newline port)
      (let loop ((l (bbs->code-list (proc-obj-code p))) (prev-filename "") (prev-line 0))
        (if (pair? l)
            (let* ((code (car l))
                   ...
                   (filename
                      (if (and loc (eq? (vector-ref loc 0) 'file)) (vector-ref loc 1) prev-filename))
                   (line
                      (if (and loc (eq? (vector-ref loc 0) 'file)) (vector-ref loc 3) prev-line)))
              (if (or (not (string=? filename prev-filename)) (not (= line prev-line)))
                  (begin
                    (display "#line " port)
                    (display line port)
...
(define (write-gvm-instr gvm-instr port)
  (define (write-closure-parms parms)
    ...
    (let ((len (+ 1 (write-gvm-opnd (closure-parms-loc parms) port))))
      ...
      (let ((len (+ len (+ 4 (write-gvm-lbl (closure-parms-lbl parms) port)))))
        (+ len (write-terminated-opnd-list (closure-parms-opnds parms) port)))))
  ...
  (define (write-param-pattern gvm-instr port)
    (let ((len
             (if (not (= (label-entry-min gvm-instr) (label-entry-nb-parms gvm-instr)))
                 (let ((len (write-returning-len (label-entry-min gvm-instr) port)))
                   (display "-" port)
                   (+ len 1))
                 0)))
      (let ((len (+ len (write-returning-len (label-entry-nb-parms gvm-instr) port))))
        (if (label-entry-rest? gvm-instr) (begin (display "+" port) (+ len 1)) len))))
    ...
    (case (gvm-instr-type gvm-instr)
      ((label)
        (let ((len (write-gvm-lbl (label-lbl-num gvm-instr) port)))
          ...
          (let ((len
                   (+ len
                      (+ 1 (write-returning-len (frame-size (gvm-instr-frame gvm-instr)) port)))))
            (case (label-type gvm-instr)
              ...
              (else (compiler-internal-error "write-gvm-instr, unknown label type"))))))
      ((apply)
        (display "  " port)
        (let ((len
                 (+ 2
                    (if (apply-loc gvm-instr)
                        (let ((len (write-gvm-opnd (apply-loc gvm-instr) port)))
                          (display " = " port)
                          (+ len 3))
                        0))))
          (+ len
             (write-prim-applic (apply-prim gvm-instr) (apply-opnds gvm-instr) port))))
      ((copy)
        (display "  " port)
      ...
      ((ifjump)
        ...
        (let ((len
                 (+ 5
                    (write-prim-applic (ifjump-test gvm-instr) (ifjump-opnds gvm-instr) port))))
          (let ((len
                   (+ len
            ...
            (let ((len
                     (+ len (write-returning-len (frame-size (gvm-instr-frame gvm-instr)) port))))
              (display " " port)
              (let ((len (+ len (+ 1 (write-gvm-lbl (ifjump-true gvm-instr) port)))))
                (display " else " port)
      ...
      ((jump)
        ...
        (let ((len
          ...
          (let ((len
                   (+ len (write-returning-len (frame-size (gvm-instr-frame gvm-instr)) port))))
            (display " " port)
            (let ((len (+ len (+ 1 (write-gvm-opnd (jump-opnd gvm-instr) port)))))
              (+ len
                 (if (jump-nb-args gvm-instr)
                     (begin
                       (display " " port)
                       (+ 1 (write-returning-len (jump-nb-args gvm-instr) port)))
                     0))))))
      (else
        (compiler-internal-error "write-gvm-instr, unknown 'gvm-instr':" gvm-instr))))
  (define (spaces n)
    (if (> n 0)
        (if (> n 7)
        ...
        (begin
          ...
          (cond
            ((eq? var closure-env-var)
              (write (map (lambda (var) (var-name var)) (frame-closed frame)) port))
    ...
    (let ((live (frame-live frame)))
      (or (set-member? var live)
         (and (eq? var closure-env-var)
              (not (set-empty? (set-intersection live (list->set (frame-closed frame)))))))))
    ...
    (if (pair? l)
        ...
        (let loop2 ((i 0) (l (frame-regs frame)) (sep sep))
          (if (pair? l)
              (let ((var (car l)))
                (if (live? var)
                    (begin (write-var var (make-reg i) sep) (loop2 (+ i 1) (cdr l) " "))
                    (loop2 (+ i 1) (cdr l) sep))))))))
    ...
    (cond
      ...
      ((clo? gvm-opnd)
        (let ((len (write-gvm-opnd (clo-base gvm-opnd) port)))
          (display "(" port)
          (let ((len (+ len (+ 1 (write-returning-len (clo-index gvm-opnd) port)))))
            (display ")" port)
            (+ len 1))))
      ...
      (else
        (compiler-internal-error "write-gvm-opnd, unknown 'gvm-opnd':" gvm-opnd))))
  (write-opnd))
...
(define (write-gvm-obj val port)
  (cond
    ((false-object? val) (display "#f" port) 2)
...
(define (make-target version name)
  ...
  (if (not (= version current-target-version))
      (compiler-internal-error "make-target, version of target package is not current"
                               name))
...
(define (cf source target-name . opts)
  (let* ((dest (file-root source))
         ...
         (result
            (compile-program2 (list **include-sym source)
...
(define (compile-program2 program target-name opts module-name dest info-port)
  (define (compiler-body)
    (if (not (valid-module-name? module-name))
        (compiler-error "Invalid characters in file name (must be a symbol with no \"#\")")
        (begin
          (ptree.begin! info-port)
          (virtual.begin!)
          (select-target! target-name info-port)
          (parse-program (list (expression->source (wrap-program program) #f))
                         ...
                         (lambda (lst env c-intf)
                           (let ((parsed-program
                             ...
                             (let ((module-init-proc
                               ...
                               (if (memq 'gvm opts)
                                   (let ((gvm-port (open-output-file (string-append dest ".gvm"))))
  ...
  (let ((successful (with-exception-handling compiler-body)))
    (if info-port
        (if successful
            (begin (display "Compilation finished." info-port) (newline info-port))
            (begin
    ...
    (define (report title pred? vars wrote-something?)
      (if (pair? vars)
          (let ((var (car vars)))
            (if (pred? var)
                (begin
                  (if (not wrote-something?) (begin (display " ") (display title) (newline)))
                  (let loop1 ((l (var-refs var)) (r? #f) (c? #f))
                    (if (pair? l)
                        ...
                        (let loop2 ((l (var-sets var)) (d? #f) (a? #f))
                          (if (pair? l)
                              (if (set? (car l)) (loop2 (cdr l) d? #t) (loop2 (cdr l) #t a?))
                              (begin
                                (display "  [")
  ...
  (let* ((entry-lbl (bbs-new-lbl! *bbs*))
    ...
    (let loop1 ((l (c-intf-procs c-intf)))
      (if (not (null? l))
          (let* ((x (car l))
            ...
            (add-constant-var var (make-obj (make-proc-obj name #t #f 0 #t '() '(#f))))
            (loop1 (cdr l)))))
    (let loop2 ((l program))
      (if (not (null? l))
          (let ((node (car l)))
                ...
                (let* ((var (def-var node)) (val (global-val var)))
                  (if (and val (prc? val))
                      (add-constant-var var
                                        (make-obj (make-proc-obj (symbol->string (var-name var))
                                                                 #t
                                                                 #f
                                                                 (call-pattern val)
                                                                 #t
    ...
    (let loop3 ((l program))
      (if (null? l)
          ...
          (let ((node (car l)))
            (if (def? node)
                (begin (gen-define (def-var node) (def-val node) info-port) (loop3 (cdr l)))
                (if (null? (cdr l))
                    (gen-node node ret-var-set 'tail)
                    (begin (gen-node node ret-var-set 'need) (loop3 (cdr l))))))))
    (let loop4 ()
      (if (pair? proc-queue)
    ...
    (bbs-purify! *bbs*)
    (let ((proc
             (make-proc-obj (string-append "#!" module-name) #t *bbs* '(0) #t '() '(#f))))
      (set! *bb* '())
      (set! *bbs* '())
      (set! *global-env* '())
      (set! proc-queue '())
      ...
      (let* ((p-bbs *bbs*)
             ...
             (bb1
                (make-bb (make-label-entry lbl1
                                           ...
                                           (source-comment node))
                         bbs))
             (bb2 (make-bb (make-label-simple lbl2 frame (source-comment node)) bbs)))
        (define (do-body)
        ...
        (let* ((x (assq var constant-vars))
               (proc
                  (if x
                      (let ((p (cdr x))) (proc-obj-code-set! (obj-val p) bbs) p)
                      (make-obj (make-proc-obj (symbol->string (var-name var))
                                               #f
                                               bbs
                                               (call-pattern node)
                                               ...
                                               '()
                                               '(#f))))))
          (put-copy proc (make-glo (var-name var)) #f ret-var-set (source-comment node))))
...
(define (entry-context proc closed)
  ...
  (let* ((parms (prc-parms proc))
         (pc
            (target.label-info (prc-min proc)
                               (length parms)
                               (prc-rest proc)
                               (not (null? closed))))
         (fs (pcontext-fs pc))
                 ...
                 ((reg? loc)
                   (let ((i (reg-num loc)))
                     (if (<= i target.nb-regs)
                         ...
                         (compiler-internal-error "entry-context, reg out of bound in back-end's pcontext"))))
                 ((stk? loc)
                   (let ((i (stk-num loc)))
                     (if (<= i fs)
                         (nth-after slots-list (- fs i))
                         (compiler-internal-error "entry-context, stk out of bound in back-end's pcontext"))))
                 (else
                   (compiler-internal-error "entry-context, loc other than reg or stk in back-end's pcontext")))))
        (if (eq? (car x) empty-var)
            (set-car! x var)
            (compiler-internal-error "entry-context, duplicate location in back-end's pcontext"))))
    (let loop ((l (pcontext-map pc)))
      (if (not (null? l))
          (let* ((couple (car l)) (name (car couple)) (loc (cdr couple)))
            (cond
              ((eq? name 'return) (assign-var-to-loc ret-var loc))
              ((eq? name 'closure-env) (assign-var-to-loc closure-env-var loc))
              (else (assign-var-to-loc (list-ref parms (- name 1)) loc)))
            (loop (cdr l)))))
    (make-context fs slots-list regs-list closed (entry-poll) #f)))
(define (get-var opnd)
  (cond
    ((glo? opnd) (env-lookup-global-var *global-env* (glo-name opnd)))
    ((reg? opnd) (list-ref regs (reg-num opnd)))
    ((stk? opnd) (list-ref slots (- nb-slots (stk-num opnd))))
    (else
      (compiler-internal-error "get-var, location must be global, register or stack slot"))))
(define (put-var opnd new)
  (define (put-v opnd new)
    (cond
      ((reg? opnd) (set! regs (replace-nth regs (reg-num opnd) new)))
      ((stk? opnd) (set! slots (replace-nth slots (- nb-slots (stk-num opnd)) new)))
      (else
        (compiler-internal-error "put-var, location must be register or stack slot, for var:"
                                 (var-name new)))))
  (if (eq? new ret-var)
...
(define (replace-nth l i v)
  (if (null? l)
      ...
      (if (= i 0) (cons v (cdr l)) (cons (car l) (replace-nth (cdr l) (- i 1) v)))))
...
(define (dead-regs live)
  (let ((live-v (live-vars live)))
    (define (loop s l i)
      (cond
        ((>= i target.nb-regs) (list->set (reverse s)))
                    ...
                    (if n
                        ...
                        (let ((n (pos-in-list var closed)))
                          (if n
                              ...
                              (compiler-internal-error "var->opnd, variable is not accessible:"
                                                       (var-name var))))))))))))
  ...
  (let* ((lbl1 (bbs-new-lbl! *bbs*))
         ...
         (bb1
            (make-bb (make-label-entry lbl1
                                       ...
                                       (source-comment proc))
                     *bbs*))
         (bb2 (make-bb (make-label-simple lbl2 frame (source-comment proc)) *bbs*)))
    (context-entry-bb-set! context bb1)
    (bb-put-branch! bb1 (make-jump (make-lbl lbl2) #f #f frame #f))
...
(define (gen-node node live why)
  (cond
    ((cst? node) (gen-return (make-obj (cst-val node)) why node))
    ((ref? node)
      (let* ((var (ref-var node)) (name (var-name var)))
        (gen-return (cond
                      ((eq? why 'side) (make-obj undef-object))
                      ((global? var)
                    ...
                    node)))
    ((set? node)
      (let* ((src (gen-node (set-val node) (set-adjoin live (set-var node)) 'keep))
             (dst (var->opnd (set-var node))))
        (put-copy src dst #f live (source-comment node))
        (gen-return (make-obj undef-object) why node)))
    ((def? node)
      (compiler-internal-error "gen-node, 'def' node not at root of parse tree"))
    ...
    ((prc? node)
      (let* ((closed (not-constant-closed-vars node))
        ...
        (let ((opnd
                 (if (null? closed-list)
                     (begin (add-known-proc (lbl-num proc-lbl) node) proc-lbl)
                     (begin
                       (dealloc-slots (- nb-slots
                                         (stk-num (highest-live-slot (set-union closed live)))))
                       (push-slot)
                       (let ((slot (make-stk nb-slots)) (var (make-temp-var 'closure)))
                         (put-var slot var)
                         (bb-put-non-branch! *bb*
                                             (make-close (list (make-closure-parms slot
                                                                                   (lbl-num proc-lbl)
                                                                                   (map var->opnd
                                                                                        closed-list)))
    ...
    (else
      (compiler-internal-error "gen-node, unknown parse tree node type:" node))))
(define (gen-return opnd why node)
  (cond
    ((eq? why 'tail)
      (let ((var (make-temp-var 'result)))
        (put-copy opnd target.proc-result var ret-var-set (source-comment node))
        (let ((ret-opnd (var->opnd ret-var)))
          (seal-bb (intrs-enabled? (node-decl node)) 'return)
          (dealloc-slots nb-slots)
          (bb-put-branch! *bb*
                          (make-jump ret-opnd #f #f (current-frame (set-singleton var)) #f)))))
    (else opnd)))
...
(define (predicate node live cont)
  ...
  (cond
    ((or (conj? node) (disj? node)) (predicate-conj/disj node live cont))
    ((app? node)
      (let ((proc (node->proc (app-oper node))))
        (if proc
            (let ((spec (specialize-for-call proc (node-decl node))))
              (if (and (proc-obj-test spec)
                       (nb-args-conforms? (length (app-args node)) (proc-obj-call-pat spec)))
    ...
    (predicate pre
               ...
               (lambda (true-lbl false-lbl)
                 (let ((pre-context (current-context)))
                   (set! *bb*
                     (make-bb (make-label-simple (if (conj? node) true-lbl false-lbl)
                   ...
                   (predicate alt
                              ...
                              (lambda (true-lbl2 false-lbl2)
                                (let ((alt-context (current-context)))
                                  ...
                                  (set! *bb*
                                    (make-bb (make-label-simple (if (conj? node) false-lbl true-lbl)
                                                                (current-frame live)
                                                                (source-comment alt))
                                             *bbs*))
                                  (merge-contexts-and-seal-bb alt-context
                                                              live
                                                              (intrs-enabled? (node-decl node))
                                                              'internal
                                                              (source-comment node))
                                  (bb-put-branch! *bb*
                                                  (make-jump (make-lbl (if (conj? node)
                                                                           false-lbl2
                                                                           true-lbl2))
                                                             #f
                                                             #f
...
(define (predicate-test node live test strict-pat args cont)
  (let loop ((args* args) (liv live) (vars* '()))
    (if (not (null? args*))
        ...
        (let* ((true-lbl (bbs-new-lbl! *bbs*)) (false-lbl (bbs-new-lbl! *bbs*)))
  ...
  (let ((pre (tst-pre node)) (con (tst-con node)) (alt (tst-alt node)))
    (predicate pre
               ...
               (lambda (true-lbl false-lbl)
                 (let ((pre-context (current-context))
                       (true-bb
                          (make-bb (make-label-simple true-lbl
                                                      (current-frame (set-union live
                                                                                (free-variables con)))
                                                      (source-comment con))
                                   *bbs*))
                       (false-bb
                          (make-bb (make-label-simple false-lbl
                                                      (current-frame (set-union live
                                                                                (free-variables alt)))
                                                      (source-comment alt))
                                   *bbs*)))
                             ...
                             (let ((next-lbl (bbs-new-lbl! *bbs*)) (alt-bb *bb*))
                               (if (> (context-nb-slots con-context) nb-slots)
                                   ...
                                   (let ((alt-context (current-context)))
                                     ...
                                     (let ((con-context* (current-context)))
                                       ...
                                       (set! *bb* alt-bb)
                                       (merge-contexts-and-seal-bb con-context*
                                                                   live-after
                                                                   (intrs-enabled? (node-decl node))
                                                                   'internal
                                                                   (source-comment node)))))
                               (let ((frame (current-frame live-after)))
                                 ...
                                 (bb-put-branch! alt-bb
                                                 (make-jump (make-lbl next-lbl) #f #f frame #f))
                                 (set! *bb*
                                   (make-bb (make-label-simple next-lbl frame (source-comment node))
                                            *bbs*))
                                 target.proc-result)))))))))))
...
(define (merge-contexts-and-seal-bb other-context live poll? where comment)
  (let ((live-v (live-vars live))
      ...
      (if (>= i 0)
          (let ((other-var (reg->var other-regs i)) (var (reg->var regs i)))
            (if (and (not (eq? var other-var)) (set-member? other-var live-v))
                (let ((r (make-reg i)))
                  (put-var r empty-var)
                  (if (not (or (not (set-member? var live-v)) (memq var regs) (memq var slots)))
                      (let ((top (make-stk (+ nb-slots 1)))) (put-copy r top var live-v comment)))
          ...
          (let ((other-var (stk->var other-slots i)) (var (stk->var slots i)))
            (if (and (not (eq? var other-var)) (set-member? other-var live-v))
                (let ((s (make-stk i)))
                  ...
                  (if (not (or (not (set-member? var live-v)) (memq var regs) (memq var slots)))
                      (let ((top (make-stk (+ nb-slots 1)))) (put-copy s top var live-v comment)))
                  (put-copy (var->opnd other-var) s other-var live-v comment))
                (if (> i nb-slots)
                    (let ((top (make-stk (+ nb-slots 1))))
                      (put-copy (make-obj undef-object) top empty-var live-v comment))))
            (loop2 (+ i 1)))))
    ...
    (if (not (eq? entry-bb other-entry-bb))
        (compiler-internal-error "merge-contexts-and-seal-bb, entry-bb's do not agree"))))
(define (seal-bb poll? where)
    ...
    (let loop ((i 0) (l1 (bb-non-branch-instrs *bb*)) (l2 '()))
      (if (< i split-point)
          ...
          (let* ((label-instr (bb-label-instr *bb*))
                 ...
                 (frame
                    (gvm-instr-frame (car (my-last-pair (cons label-instr non-branch-instrs1)))))
                 (prec-bb (make-bb label-instr *bbs*))
                 (new-lbl (bbs-new-lbl! *bbs*)))
            (bb-non-branch-instrs-set! prec-bb non-branch-instrs1)
            (bb-put-branch! prec-bb (make-jump (make-lbl new-lbl) #f #t frame #f))
  ...
  (define (impose-polling-constraints)
    (let ((n (+ (length (bb-non-branch-instrs *bb*)) 1)) (delta (poll-delta poll)))
      (if (> (+ delta n) poll-period)
          (begin (poll-at (max (- poll-period delta) 0)) (impose-polling-constraints)))))
  ...
  (let* ((n (+ (length (bb-non-branch-instrs *bb*)) 1))
    ...
    (if (and poll?
             (case where
               ...
               ((internal) #f)
               (else (compiler-internal-error "seal-bb, unknown 'where':" where))))
        (poll-at-end)
    ...
    (let ((needed (set-union live (free-variables alt)))
      ...
      (define (alternative con-lbl alt-lbl)
        (let* ((pre-context (current-context))
               ...
               (alt-live (set-union live (free-variables alt)))
               (con-bb
                  (make-bb (make-label-simple con-lbl (current-frame con-live) (source-comment alt))
                           *bbs*))
               (alt-bb
                  (make-bb (make-label-simple alt-lbl (current-frame alt-live) (source-comment alt))
                           *bbs*)))
          (if bool?
              (begin
                (set! *bb* con-bb)
            ...
            (let ((alt-opnd (gen-node alt live why)))
              (if (eq? why 'tail)
                  (begin
                    (restore-context con-context)
                    (set! *bb* con-bb)
                    (let ((ret-opnd (var->opnd ret-var)) (result-set (set-singleton result-var)))
                      (seal-bb (intrs-enabled? (node-decl node)) 'return)
                      (dealloc-slots nb-slots)
                      (bb-put-branch! *bb* (make-jump ret-opnd #f #f (current-frame result-set) #f))))
                  (let ((alt-context* (current-context)) (alt-bb* *bb*))
                    (restore-context con-context)
                    (set! *bb* con-bb)
                    (seal-bb (intrs-enabled? (node-decl node)) 'internal)
                    (let ((con-context* (current-context)) (next-lbl (bbs-new-lbl! *bbs*)))
                      (restore-context alt-context*)
                      (set! *bb* alt-bb*)
                      ...
                      (let ((frame (current-frame (set-adjoin live result-var))))
                        (bb-put-branch! *bb* (make-jump (make-lbl next-lbl) #f #f frame #f))
                        (bb-put-branch! con-bb (make-jump (make-lbl next-lbl) #f #f frame #f))
                        (set! *bb*
                          (make-bb (make-label-simple next-lbl frame (source-comment node)) *bbs*))
                        target.proc-result))))))))
...
(define (gen-call node live why)
  (let* ((oper (app-oper node)) (args (app-args node)) (nb-args (length args)))
    (if (and (prc? oper) (not (prc-rest oper)) (= (length (prc-parms oper)) nb-args))
              ...
              (let ((frame-start (stk-num (highest-live-slot live-after))))
                (let loop1 ((l in-stk) (liv live-after) (i (+ frame-start 1)))
                  (if (not (null? l))
                      (let ((arg (car l))
                            (slot (make-stk i))
                            (needed (set-union (stk-live-vars liv (cdr l) why) live-for-regs)))
                        (if arg
                            (let ((var
                                     (if (and (eq? arg 'return) (eq? why 'tail))
                                         ret-var
                              ...
                              (save-opnd-to-stk (if (eq? arg 'return)
                                                ...
                                                var
                                                needed
                                                (source-comment (if (eq? arg 'return) node arg)))
                              (loop1 (cdr l) (set-adjoin liv var) (+ i 1)))
                            (begin
                              (if (> i nb-slots)
                                  (put-copy (make-obj undef-object)
                                            slot
                                            empty-var
                                            liv
                                            (source-comment node)))
                            ...
                            (let* ((arg (car (car l)))
                                   ...
                                   (opnd
                                      (if (eq? arg 'return)
                                          ...
                                          (gen-node arg needed 'need))))
                              (if (eq? pos 'operator)
                                  (if (and (ref? arg) (not (or (obj? opnd) (lbl? opnd))))
                                      (loop2 (cdr l)
                                      ...
                                      (begin
                                        (save-arg opnd
                                                  ...
                                                  needed
                                                  (source-comment (if (eq? arg 'return) node arg)))
                                        (loop2 (cdr l) (set-adjoin liv var) reg-map var)))
                                  (let ((reg (make-reg pos)))
                                    (if (all-args-trivial? (cdr l))
                                        (save-opnd-to-reg opnd
                                                          ...
                                                          var
                                                          needed
                                                          (source-comment (if (eq? arg 'return)
                                                                              node
                                                                              arg)))
                                        (save-in-slot opnd
                                                      var
                                    ...
                                    (let ((result-var (make-temp-var 'result)))
                                      ...
                                      (if return-lbl
                                          (begin
                                            ...
                                            (set! *bb*
                                              (make-bb (make-label-return return-lbl
                                                                          (current-frame (set-adjoin live
                                                                                                     result-var))
                                                                          (source-comment node))
                                                       *bbs*))))
                                      target.proc-result))))))))))))))
...
(define (save-regs regs live comment)
  (for-each (lambda (i) (save-opnd (make-reg i) live comment)) (set->list regs)))
...
(define (all-args-trivial? l)
  (if (null? l)
      #t
      (let ((arg (car (car l))))
        (or (eq? arg 'return) (and (trivial? arg) (all-args-trivial? (cdr l)))))))
...
(define (node->proc node)
  (cond
    ((cst? node) (if (proc-obj? (cst-val node)) (cst-val node) #f))
    ((ref? node)
               ...
               (cond
                 ((reg? loc)
                   (let ((i (reg-num loc)))
                     (if (<= i target.nb-regs)
                         (nth-after regs-list i)
                         (compiler-internal-error "jump-state, reg out of bound in back-end's pcontext"))))
                 ((stk? loc)
                   (let ((i (stk-num loc)))
                     (if (<= i fs)
                         (nth-after slots-list (- i 1))
                         (compiler-internal-error "jump-state, stk out of bound in back-end's pcontext"))))
                 (else
                   (compiler-internal-error "jump-state, loc other than reg or stk in back-end's pcontext")))))
        (if (not (car x))
            (set-car! x var)
            (compiler-internal-error "jump-state, duplicate location in back-end's pcontext"))))
    (let loop ((l (pcontext-map pc)))
      (if (not (null? l))
          (let* ((couple (car l)) (name (car couple)) (loc (cdr couple)))
            (cond
              ((eq? name 'return) (assign-node-to-loc 'return loc))
    ...
    (cond
      ((null? nodes)
        (let ((p1 (reverse part1)) (p2 (free-vars-order part2)))
          (cond
            ((not oper) (append p1 p2))
            ((trivial? oper) (append p1 p2 (list (cons oper 'operator))))
      ...
      ((or (eq? (car nodes) 'return) (trivial? (car nodes)))
        (loop (cdr nodes) (+ pos 1) part1 (cons (cons (car nodes) pos) part2)))
      (else (loop (cdr nodes) (+ pos 1) (cons (cons (car nodes) pos) part1) part2))))
  (loop nodes 0 '() '()))
  ...
  (let ((bins '()) (ordered-args '()))
    ...
    (define (remove-free-vars! x)
      (let loop ((l (set->list x)))
        (if (not (null? l))
            (let ((y (assq (car l) bins))) (set-cdr! y (- (cdr y) 1)) (loop (cdr l))))))
    ...
    (for-each (lambda (x) (for-each add-to-bin! (set->list (free-v (car x))))) l)
  ...
  (cond
    ...
    ((eq? (car slots) 'return)
      (stk-live-vars (if (eq? why 'tail) (set-adjoin live ret-var) live)
                     ...
                     why))
    (else
      (stk-live-vars (set-union live (free-variables (car slots))) (cdr slots) why))))
(define (gen-let vars vals node live why)
  (let ((var-val-map (pair-up vars vals))
    ...
    (define (closed-vars var const-proc-vars)
      (set-difference (not-constant-closed-vars (var->val var)) const-proc-vars))
    ...
    (define (compute-const-proc-vars proc-vars)
      (let loop1 ((const-proc-vars proc-vars))
        (let ((new-const-proc-vars
                 (set-keep (lambda (x) (no-closed-vars? x const-proc-vars)) const-proc-vars)))
    ...
    (let* ((proc-vars (set-keep proc-var? var-set))
           ...
           (clo-vars (set-keep (lambda (x) (closed-vars? x const-proc-vars)) proc-vars))
           (clo-vars-list (set->list clo-vars)))
      (for-each (lambda (proc-var)
      ...
      (let ((non-clo-vars-list
            ...
            (liv
               (set-union live
                          (apply set-union
                                 (map (lambda (x) (closed-vars x const-proc-vars)) clo-vars-list))
                          (free-variables node))))
          ...
          (if (not (null? vars*))
              (let* ((var (car vars*))
                ...
                (if (var-useless? var)
                    ...
                    (save-val (gen-node val needed 'need) var needed (source-comment val)))
                (loop2 (cdr vars*)))))
                ...
                (if (not (null? l))
                    (begin
                      ...
                      (let ((var (car l)) (slot (make-stk nb-slots)))
                        ...
                        (loop3 (cdr l))))))
              (bb-put-non-branch! *bb*
                                  (make-close (map (lambda (var)
                                                     (let ((closed-list
                                                              (sort-variables (set->list (closed-vars var
                                                                                                      const-proc-vars)))))
                                                       (if (null? closed-list)
                                                           (compiler-internal-error "gen-let, no closed variables:"
                                                                                    (var-name var))
                                                           (make-closure-parms (var->opnd var)
                                                                               (lbl-num (schedule-gen-proc (var->val var)
                                                                                                           closed-list))
                                                                               (map var->opnd
                                                                                    closed-list)))))
                                                   clo-vars-list)
...
(define (save-arg opnd var live comment)
  (if (glo? opnd) (add-constant-var var opnd) (save-val opnd var live comment)))
(define (save-val opnd var live comment)
  (cond
    ((or (obj? opnd) (lbl? opnd)) (add-constant-var var opnd))
...
(define (save-var opnd var live comment)
  (cond
    ((or (obj? opnd) (lbl? opnd)) (add-constant-var var opnd) var)
...
(define (vals-live-vars live vals)
  (if (null? vals)
      live
      (vals-live-vars (set-union live (free-variables (car vals))) (cdr vals))))
(define (gen-fut node live why)
  (let* ((val (fut-val node))
         ...
         (live-after live)
         (live-starting-task (set-adjoin (set-union live-after clo-vars) ret-var*))
    ...
    (let ((frame-start (stk-num (highest-live-slot live-after))))
                    ...
                    (if (>= j 0)
                        (if (or (>= j (length regs))
                            ...
                            (let ((reg (make-reg j)))
                              ...
                              (loop1 rest i))
                            (loop2 (- j 1)))
                        (let ((slot (make-stk (+ frame-start (+ i 1)))) (needed (list->set rest)))
                          (if (and (or (> (stk-num slot) nb-slots)
                              ...
                              (save-opnd slot live-starting-task (source-comment node)))
                          (put-copy (var->opnd var)
                                    slot
                                    var
                                    live-starting-task
      ...
      (bb-put-branch! *bb*
                      (make-jump (make-lbl task-lbl) #f #f (current-frame live-starting-task) #f))
      (let ((task-context
        ...
        (set! *bb*
          (make-bb (make-label-task-entry task-lbl
        ...
        (let ((result-var (make-temp-var 'future)))
          ...
          (put-var target.proc-result result-var)
          (set! *bb*
            (make-bb (make-label-task-return return-lbl
...
(define (asm-display obj asm-port)
  (if (eqv? obj char-newline)
      (begin
        (set! asm-output (cons (apply string-append (reverse asm-line)) asm-output))
        (set! asm-line '()))
      (set! asm-line
        (cons (cond
                ((string? obj) obj)
        ...
        (if (symbol-object? obj)
            ...
            (let ((m (ofile-add-obj obj)))
              (if m
                  ...
                  (begin (ofile-lsym prim-proc-object-bits (proc-obj-name obj)))))))))
    ...
    (cond
      ...
      ((pair? x)
        (let loop ((l1 (cdr x)) (l2 (list (obj->string (car x)) "(")))
          (cond
            ((pair? l1) (loop (cdr l1) (cons (obj->string (car l1)) (cons " " l2))))
            ((null? l1) (apply string-append (reverse (cons ")" l2))))
            (else
              (apply string-append
                     (reverse (cons ")" (cons (obj->string l1) (cons " . " l2)))))))))
      (else (compiler-internal-error "ofile-stat, can't convert to string 'x'" x))))
...
(define (obj-subtype obj)
  (cond
    ((symbol-object? obj) 'symbol)
    ((number? obj)
      (cond
        ((and (integer? obj) (exact? obj)) 'bignum)
        ((and (rational? obj) (exact? obj)) 'ratnum)
        (else 'cpxnum)))
    ((vector? obj) 'vector)
    ((string? obj) 'string)
    (else (compiler-internal-error "obj-subtype, unknown object 'obj'" obj))))
...
(define (obj-encoding obj)
  (case (obj-type obj)
    ...
    ((special)
      (make-encoding (cond
                       ((false-object? obj) data-false)
                       ((undef-object? obj) data-undef)
                       ((eq? obj #t) data-true)
                       ((null? obj) data-null)
                       ((char? obj) (character-encoding obj))
                       (else
                         (compiler-internal-error "obj-encoding, unknown SPECIAL object 'obj'" obj)))
                     type-special))
    (else #f)))
...
(define (asm-assemble! debug-info)
  ...
  (let ((code-list (queue->list asm-code-queue))
    ...
    (let* ((fix-list
                    ...
                    (let ((part (car l)) (rest (cdr l)))
                      (if (pair? part)
                          (case (car part)
                            ...
                            (else
                              (compiler-internal-error "asm-assemble!, unknown code list element"
                                                       part)))
                          (loop rest (+ len 2) x))))))
           (lbl-list
        ...
        (let loop ((l code-list))
          (if (not (null? l))
              (let ((part (car l)) (rest (cdr l)))
                (if (pair? part)
                    (case (car part)
                      ((brab) (set-cdr! (cdr part) (cdr (assq (cddr part) lbl-list))))
                      ((wrel) (set-car! (cdr part) (cdr (assq (cadr part) lbl-list))))
                      ((lrel) (set-car! (cdr part) (cdr (assq (cadr part) lbl-list))))))
                (loop rest)))))
          ...
          (if (not (null? l))
              (let* ((first (car l))
                ...
                (case (car part)
                  ...
                  (else
                    (compiler-internal-error "assign-loc-to-labels, unknown code list element"
                                             part)))))))
                  ...
                  ((brab)
                    (let ((dist (- (cadr (cddr part)) (+ cur-loc 2))))
                      (if (or (< dist -128) (> dist 127) (= dist 0))
                          (begin (set-car! part 'braw) (loop #t rest (+ cur-loc 2)))
      ...
      (define (write-code)
        (let ((proc-len
                 (+ (cadr (cdr (assq const-lbl lbl-list))) (* (length const-list) 4))))
          (if (>= proc-len 32768)
              (compiler-limitation-error "procedure is too big (32K bytes limit per procedure)"))
                      ...
                      (case (car part)
                        ...
                        (else
                          ...
                          (case (car part)
                            ...
                            ((m68020-proc) (ofile-word m68020-proc-code-tag) (loop1 rest loc))
                            ((m68881-proc) (ofile-word m68881-proc-code-tag) (loop1 rest loc))
                            ((stat)
                              (ofile-word stat-tag)
                              (ofile-stat (cdr part))
...
(define (emit-eor.l opnd1 opnd2)
  (cond
    ((imm? opnd1)
      ...
      (opnd-ext-wr-long opnd2))
    (else
      (asm-word (+ 45440 (+ (* (dreg-num opnd1) 512) (opnd->mode/reg opnd2))))
      (opnd-ext-wr-long opnd2)))
  (if ofile-asm?
  ...
  (cond
    ...
    (else
      (let ((mode (if (dreg? opnd2) 49280 49536))
        ...
        (if (dreg? opnd2) (opnd-ext-rd-long other) (opnd-ext-wr-long other)))))
  (if ofile-asm?
  ...
  (cond
    ...
    (else
      (let ((mode (if (dreg? opnd2) 49216 49472))
            ...
            (other (if (dreg? opnd2) opnd1 opnd2)))
        (asm-word (+ mode (+ (* reg 512) (opnd->mode/reg other))))
        (if (dreg? opnd2) (opnd-ext-rd-word other) (opnd-ext-wr-word other)))))
  (if ofile-asm?
...
(define (emit-add.l opnd1 opnd2)
  (cond
    ((areg? opnd2)
      (asm-word (+ 53696 (+ (* (areg-num opnd2) 512) (opnd->mode/reg opnd1))))
      (opnd-ext-rd-long opnd1))
    ((imm? opnd1)
...
(define (emit-add.w opnd1 opnd2)
  (cond
    ((areg? opnd2)
      (asm-word (+ 53440 (+ (* (areg-num opnd2) 512) (opnd->mode/reg opnd1))))
      (opnd-ext-rd-word opnd1))
    ((imm? opnd1)
...
(define (emit-addx.w opnd1 opnd2)
  (if (dreg? opnd1)
      ...
      (asm-word (+ 53576
                   (+ (* (areg-num (pdec-areg opnd2)) 512) (areg-num (pdec-areg opnd1))))))
  (if ofile-asm?
...
(define (emit-sub.l opnd1 opnd2)
  (cond
    ((areg? opnd2)
      (asm-word (+ 37312 (+ (* (areg-num opnd2) 512) (opnd->mode/reg opnd1))))
      (opnd-ext-rd-long opnd1))
    ((imm? opnd1)
...
(define (emit-sub.w opnd1 opnd2)
  (cond
    ((areg? opnd2)
      (asm-word (+ 37056 (+ (* (areg-num opnd2) 512) (opnd->mode/reg opnd1))))
      (opnd-ext-rd-word opnd1))
    ((imm? opnd1)
...
(define (emit-cmp.l opnd1 opnd2)
  (cond
    ((areg? opnd2)
      (asm-word (+ 45504 (+ (* (areg-num opnd2) 512) (opnd->mode/reg opnd1))))
      (opnd-ext-rd-long opnd1))
    ((imm? opnd1)
    ...
    (else
      (asm-word (+ 45184 (+ (* (dreg-num opnd2) 512) (opnd->mode/reg opnd1))))
      (opnd-ext-rd-long opnd1)))
  (if ofile-asm?
...
(define (emit-cmp.w opnd1 opnd2)
  (cond
    ((areg? opnd2)
      (asm-word (+ 45248 (+ (* (areg-num opnd2) 512) (opnd->mode/reg opnd1))))
      (opnd-ext-rd-word opnd1))
    ((imm? opnd1)
    ...
    (else
      (asm-word (+ 45120 (+ (* (dreg-num opnd2) 512) (opnd->mode/reg opnd1))))
      (opnd-ext-rd-word opnd1)))
  (if ofile-asm?
...
(define (emit-cmp.b opnd1 opnd2)
  (cond
    ((imm? opnd1)
      ...
      (opnd-ext-rd-word opnd2))
    (else
      (asm-word (+ 45056 (+ (* (dreg-num opnd2) 512) (opnd->mode/reg opnd1))))
      (opnd-ext-rd-word opnd1)))
  (if ofile-asm?
...
(define (emit-lea opnd areg)
  ...
  (opnd-ext-rd-long opnd)
  (if ofile-asm? (emit-asm "lea" ofile-tab (opnd-str opnd) "," (opnd-str areg))))
...
(define (emit-fop.dx name code opnd1 opnd2)
  ...
  (asm-word (+ (if (freg? opnd1) (* (freg-num opnd1) 1024) 21504)
               (* (freg-num opnd2) 128)
               code))
  (opnd-ext-rd-long opnd1)
  (if ofile-asm?
...
(define (emit-fmov.dx opnd1 opnd2)
  (emit-fmov (if (and (freg? opnd1) (freg? opnd2)) (* (freg-num opnd1) 1024) 21504)
             opnd1
             opnd2)
  (if ofile-asm?
      (emit-asm (if (and (freg? opnd1) (freg? opnd2)) "fmovex" "fmoved")
                ofile-tab
                (opnd-str opnd1)
                ","
                (opnd-str opnd2))))
...
(define (opnd-str opnd)
  (cond
    ((dreg? opnd)
      (vector-ref '#("d0" "d1" "d2" "d3" "d4" "d5" "d6" "d7") (dreg-num opnd)))
    ((areg? opnd)
      (vector-ref '#("a0" "a1" "a2" "a3" "a4" "a5" "a6" "sp") (areg-num opnd)))
    ((ind? opnd)
    ...
    ((pcr? opnd)
      (let ((lbl (pcr-lbl opnd)) (offs (pcr-offset opnd)))
        (if (= offs 0)
            ...
            (string-append "L" (number->string lbl) "+" (number->string offs)))))
    ((imm? opnd) (string-append "#" (number->string (imm-val opnd))))
    ((glob? opnd) (string-append "GLOB(" (symbol->string (glob-name opnd)) ")"))
    ((freg? opnd)
...
(define (label-info min-args nb-parms rest? closed?)
  (let ((nb-stacked (max 0 (- nb-parms nb-arg-regs))))
    (define (location-of-parms i)
      (if (> i nb-parms)
          '()
          (cons (cons i (if (> i nb-stacked) (make-reg (- i nb-stacked)) (make-stk i)))
                (location-of-parms (+ i 1)))))
    (let ((x (cons (cons 'return 0) (location-of-parms 1))))
      (make-pcontext nb-stacked
                     (if closed? (cons (cons 'closure-env (make-reg (+ nb-arg-regs 1))) x) x)))))
...
(define (flonum->inexact-exponential-format x)
  (define (exp-form-pos x y i)
    (let ((i*2 (+ i i)))
      (let ((z
               (if (and (not (< flonum-e-bias i*2)) (not (< x y)))
  ...
  (define (exp-form-neg x y i)
    (let ((i*2 (+ i i)))
      (let ((z
               (if (and (< i*2 flonum-e-bias-minus-1) (< x y))
  ...
  (if (negative? x)
      (let ((z (exp-form (- inexact-0 x)))) (set-car! z (- inexact-0 (car z))) z)
      (exp-form x)))
...
(define flonum-sign-bit 92233720368547758)
(define flonum-+m-min 45035996273704)
;(define flonum--m-min -45035996273704)
    ...
    (let loop ((prev-bb #f) (prev-gvm-instr #f) (l (bbs->code-list bbs)))
      (if (not (null? l))
          (let ((pres-bb (code-bb (car l)))
                ...
                (next-gvm-instr (if (null? (cdr l)) #f (code-gvm-instr (cadr l)))))
            (if ofile-asm? (asm-comment (car l)))
            (gen-gvm-instr prev-gvm-instr pres-gvm-instr next-gvm-instr pres-slots-needed)
            (loop pres-bb pres-gvm-instr (cdr l)))))
    (asm.end! (if debug-info?
  ...
  (case (gvm-instr-type gvm-instr)
    ((label)
      ...
      (case (label-type gvm-instr)
        ...
        ((entry)
          (gen-label-entry (label-lbl-num gvm-instr)
    ...
    ((apply)
      (gen-apply (apply-prim gvm-instr)
    ...
    ((ifjump)
      (gen-ifjump (ifjump-test gvm-instr)
                  (ifjump-opnds gvm-instr)
                  (ifjump-true gvm-instr)
                  (ifjump-false gvm-instr)
                  (ifjump-poll? gvm-instr)
                  (if (and next-gvm-instr (memq (label-type next-gvm-instr) '(simple task-entry)))
                      (label-lbl-num next-gvm-instr)
                      #f)))
    ((jump)
      (gen-jump (jump-opnd gvm-instr)
                ...
                (if (and next-gvm-instr (memq (label-type next-gvm-instr) '(simple task-entry)))
                    ...
                    #f)))
    (else
      (compiler-internal-error "gen-gvm-instr, unknown 'gvm-instr':" gvm-instr))))
...
(define (temp-in-opnd68 opnd)
  (let ((reg (reg-in-opnd68 opnd)))
    (if reg
        (cond
          ((identical-opnd68? reg dtemp1) reg)
...
(define (opnd-stat-add! type opnd)
  (cond
    ((reg? opnd) (stat-add! (list 'gvm-opnd 'reg type (reg-num opnd)) 1))
    ...
    (else (compiler-internal-error "opnd-stat-add!, unknown 'opnd':" opnd))))
              ...
              (if shift
                  (let ((m (min shift 32)))
                    (if (or (<= m 8) (identical-opnd68? reg dtemp1))
                        (let loop ((i m))
                          (if (> i 0) (begin (emit-asl.l (make-imm (min i 8)) reg) (loop (- i 8)))))
...
(define (sn-opnd68 opnd sn)
  (cond
    ((and (disp*? opnd) (identical-opnd68? (disp*-areg opnd) sp-reg))
...
(define (opnd68->true-opnd68 opnd sn)
  (if (frame-base-rel? opnd)
      (let ((slot (frame-base-rel-slot opnd)))
        (cond
          ((> slot current-fs) (adjust-current-fs 1) pdec-sp)
          ((and (= slot current-fs) (< sn current-fs)) (adjust-current-fs -1) pinc-sp)
...
(define (clo->loc68 opnd keep sn)
  (let ((base (clo-base opnd)) (offs (closed-var-offset (clo-index opnd))))
    (cond
      ((eq? base return-reg) (make-disp* (reg->reg68 base) offs))
      ((obj? base)
        (let ((areg (pick-atemp keep)))
...
(define (move-opnd68-to-loc opnd loc sn)
  (cond
    ...
    ((stk? loc)
      (let* ((loc-slot (stk-num loc))
        ...
        (if (> current-fs loc-slot)
            (make-top-of-frame (if (frame-base-rel? opnd)
    ...
    ((clo? loc)
      (let ((clo (clo->loc68 loc (temp-in-opnd68 opnd) (sn-opnd68 opnd sn))))
        (make-top-of-frame-if-stk-opnd68 opnd sn)
        (move-opnd68-to-loc68 (opnd68->true-opnd68 opnd sn) clo)))
    (else (compiler-internal-error "move-opnd68-to-loc, unknown 'loc':" loc))))
...
(define (touch-reg68-to-reg68 src dst)
  (define (trap-to-touch-handler dreg lbl)
    (if ofile-stats?
        (emit-stat '((touch 0 (determined-placeholder -1) (undetermined-placeholder 1)))))
  ...
  (define (touch-dreg-to-reg src dst)
    (let ((lbl1 (new-lbl!)))
      ...
      (emit-bne lbl1)
      (if ofile-stats?
          (emit-stat '((touch 0 (non-placeholder -1) (determined-placeholder 1)))))
  ...
  (cond
    ((dreg? src) (touch-dreg-to-reg src dst))
  ...
  (if (reg? opnd)
      (let ((reg68 (reg->reg68 opnd)))
        (if (reg? loc)
            ...
            (begin (touch-reg68-to-reg68 reg68 reg68) (move-opnd68-to-loc reg68 loc sn))))
      (if (reg? loc)
    ...
    (let loop1 ((save1 '()) (save2 #f) (regs (frame-regs frame)) (i 0))
      (if (pair? regs)
          ...
          (let ((order (sort-list save1 (lambda (x y) (< (car x) (car y))))))
            (let ((slots
                     (append (map cdr order)
                             (adjust-slots (frame-slots frame) (- current-fs (frame-size frame)))))
              ...
              (if save2
                  (begin
                    (emit-move.l (car save2)
                                 (make-disp* sp-reg (* pointer-size (- current-fs ret-slot))))
                    (set-slot! slots ret-slot (cdr save2))))
              (if (> (length order) 2)
                  (begin (emit-movem.l reg-list pdec-sp) (trap) (emit-movem.l pinc-sp reg-list))
                  (let loop2 ((l (reverse reg-list)))
                    (if (pair? l)
                        ...
                        (trap))))
              (if save2
                  (emit-move.l (make-disp* sp-reg (* pointer-size (- current-fs ret-slot)))
                               (car save2)))
              (emit-label lbl)))))))
  ...
  (let* ((nb-parms* (if rest? (- nb-parms 1) nb-parms))
    ...
    (if closed?
        (let ((closure-reg (reg-num->reg68 (+ nb-arg-regs 1))))
          ...
          (if (or (and (<= min 1) (<= 1 nb-parms*)) (and (<= min 2) (<= 2 nb-parms*)))
    ...
    (cond
      (rest?
        (emit-trap1 (if closed? rest-params-closed-trap rest-params-trap)
                    (list min nb-parms*))
      ...
      ((= min nb-parms*)
        (emit-trap1 (if closed? wrong-nb-arg1-closed-trap wrong-nb-arg1-trap)
                    (list nb-parms*))
        (if (not closed?) (emit-lbl-ptr lbl)))
      (else
        (emit-trap1 (if closed? wrong-nb-arg2-closed-trap wrong-nb-arg2-trap)
                    (list min nb-parms*))
        (if (not closed?) (emit-lbl-ptr lbl))))
              ...
              (let ((nb-stacked (if (<= i nb-arg-regs) 0 (- i nb-arg-regs))))
                ...
                (let loop2 ((j 1))
                  (if (and (<= j nb-arg-regs)
                      ...
                      (begin (emit-move.l (reg-num->reg68 j) pdec-sp) (loop2 (+ j 1)))
                      (let loop3 ((k j))
                        (if (and (<= k nb-arg-regs) (<= k i))
                            (begin
                              (emit-move.l (reg-num->reg68 k) (reg-num->reg68 (+ (- k j) 1)))
                              (loop3 (+ k 1)))))))
                (if (> i min) (emit-bra (vector-ref optional-lbls (- nb-parms i))))
                (loop1 (- i 1))))))
    (let loop ((i min))
      (if (<= i nb-parms)
          (let ((val (if (= i nb-parms*) bits-null bits-unass)))
            ...
            (cond
              ((> (- nb-parms i) nb-arg-regs) (move-n-to-loc68 val pdec-sp))
              ((< i nb-parms)
                (move-n-to-loc68 val (reg-num->reg68 (parm->reg-num (+ i 1) nb-parms)))))
            (loop (+ i 1)))))))
...
(define (no-arg-check-entry-offset proc nb-args)
  (let ((x (proc-obj-call-pat proc)))
    (if (and (pair? x) (null? (cdr x)))
        (let ((arg-count (car x)))
          (if (= arg-count nb-args) (if (or (= arg-count 1) (= arg-count 2)) 10 14) 0))
        0)))
...
(define (gen-label-return* lbl label-descr slots extra)
  (let ((i (pos-in-list ret-var slots)))
    (if i
        ...
        (compiler-internal-error "gen-label-return*, no return address in frame"))))
  ...
  (if (= current-fs 0)
      ...
      (begin
        ...
        (let loop ((i (- current-fs 1)))
          (if (> i 0)
              (begin (emit-move.l (make-pinc atemp1) (make-disp atemp1 -8)) (loop (- i 1)))))
...
(define (gen-label-task-return lbl sn)
  ...
  (let ((slots (frame-slots exit-frame)))
    ...
    (let ((dummy-lbl (new-lbl!)) (skip-lbl (new-lbl!)))
      (gen-label-return* dummy-lbl
                         (add-first-class-label! instr-source slots exit-frame)
                         slots
                         1)
      (emit-bra skip-lbl)
      (gen-label-task-return* lbl
                              (add-first-class-label! instr-source slots exit-frame)
                              slots
                              1)
...
(define (gen-label-task-return* lbl label-descr slots extra)
  (let ((i (pos-in-list ret-var slots)))
    (if i
        (let* ((fs (length slots)) (link (- fs i)))
          (emit-label-task-return lbl entry-lbl-num (+ fs extra) link label-descr))
        (compiler-internal-error "gen-label-task-return*, no return address in frame"))))
      ...
      (let loop1 ((parms parms))
        (let ((loc (closure-parms-loc (car parms)))
          ...
          (if (= size 1)
              ...
              (emit-move.w (make-imm (+ 32768 (* (+ size 1) 4))) (make-pinc atemp2)))
...
(define (gen-ifjump* proc opnds true-lbl false-lbl poll? next-lbl)
  (let ((fs (frame-size exit-frame)))
    (define (double-branch)
      ...
      (if ofile-stats?
          (emit-stat '((gvm-instr.ifjump.fall-through 1) (gvm-instr.ifjump.double-branch 1))))
      (emit-bra true-lbl)
      (gen-deferred-code!))
    (gen-guarantee-fudge)
    (if poll? (gen-poll))
    (if next-lbl
        (cond
          ((= true-lbl next-lbl)
            (proc #t opnds false-lbl fs)
            (if ofile-stats? (emit-stat '((gvm-instr.ifjump.fall-through 1)))))
          ((= false-lbl next-lbl)
...
(define (gen-jump opnd nb-args poll? next-lbl)
  (let ((fs (frame-size exit-frame)))
    (if ofile-stats?
        (begin
          (stat-add! (list 'gvm-instr 'jump (opnd-stat opnd) nb-args (if poll? 'poll 'not-poll))
                     1)
          (jump-stat-add! opnd)
    ...
    (cond
      ...
      ((obj? opnd)
        ...
        (let ((val (obj-val opnd)))
          (if (proc-obj? val)
              (let ((num (add-object val)) (offset (no-arg-check-entry-offset val nb-args)))
                (setup-jump fs (if (<= offset 0) nb-args #f))
                (if num (emit-jmp-proc num offset) (emit-jmp-prim val offset))
                (gen-deferred-code!))
              (gen-jump* (opnd->opnd68 opnd #f fs) fs nb-args))))
      (else (if poll? (gen-poll)) (gen-jump* (opnd->opnd68 opnd #f fs) fs nb-args)))))
...
(define (make-gen-slot-ref slot type)
  (lambda (opnds loc sn)
    (let ((sn-loc (sn-opnd loc sn)) (opnd (car opnds)))
      ...
      (move-opnd68-to-loc (make-disp* atemp1 (- (* slot pointer-size) type)) loc sn))))
    ...
    (let ((sn-loc (if loc (sn-opnd loc sn) sn)))
      (let* ((first-opnd (car opnds))
        ...
        (move-opnd-to-loc68 second-opnd
                            ...
                            sn-loc)
        (if loc (if (not (eq? first-opnd loc)) (move-opnd68-to-loc atemp1 loc sn)))))))
...
(define (gen-set-car! opnds loc sn)
  (let ((sn-loc (if loc (sn-opnd loc sn) sn)))
    (let* ((first-opnd (car opnds))
      ...
      (if (and loc (not (eq? first-opnd loc))) (move-opnd68-to-loc atemp1 loc sn)))))
  ...
  (if (null? opnds)
      ...
      (let ((opnd (car opnds)) (rest (cdr opnds)))
        (cond
          ...
          (else (commut-oper gen rest loc sn self? accum-self (cons opnd accum-other)))))))
...
(define (gen-rem first-opnd second-opnd loc sn)
  (let* ((sn-loc (sn-opnd loc sn)) (sn-second-opnd (sn-opnd second-opnd sn-loc)))
    (move-opnd-to-loc68 first-opnd dtemp1 sn-second-opnd)
    (let ((opnd68 (opnd->opnd68 second-opnd #f sn-loc))
          (reg68
             (if (and (reg? loc) (not (eq? loc return-reg))) (reg->reg68 loc) false-reg)))
  ...
  (let* ((sn-loc (sn-opnd loc sn))
    ...
    (define (general-case)
      (let ((lbl1 (new-lbl!))
            ...
            (opnd68*
               (opnd68->true-opnd68 (opnd->opnd68 first-opnd #f sn-second-opnd)
                                    sn-second-opnd)))
        (move-opnd68-to-loc68 opnd68* dtemp1)
        ...
        (if (imm? o2)
            (let* ((reg68
              ...
              (let loop ((i (min (abs n) 29)))
                (if (> i 0) (begin (emit-shft (make-imm (min i 8)) reg68) (loop (- i 8)))))
              (if (< n 0) (emit-and.w (make-imm -8) reg68))
              (move-opnd68-to-loc reg68 loc sn))
            (let* ((reg68
                      (if (and (reg? loc) (not (eq? loc return-reg))) (reg->reg68 loc) dtemp1))
                   (reg68* (if (and (reg? loc) (not (eq? loc return-reg))) dtemp1 false-reg))
                   (lbl1 (new-lbl!))
                   (lbl2 (new-lbl!)))
                    ...
                    (if (imm? o2)
                        ...
                        (begin
                          (move-opnd68-to-loc68 (opnd68->true-opnd68 o2 (sn-opnd68 o1 sn-loc))
                                                dtemp1)
                          (emit-asr.l (make-imm (vector-select kind 1 3 3 2)) dtemp1)
                          (move-opnd68-to-loc68 (opnd68->true-opnd68 o1 sn-loc) atemp1)
                          (if (and (identical-opnd68? reg68 dtemp1) (not (obj-vector? kind)))
                              (begin (emit-move.l dtemp1 atemp2) (make-inx atemp1 atemp2 offset))
  ...
  (lambda (opnds loc sn)
    (let ((sn-loc (if loc (sn-opnd loc sn) sn)))
      (let ((first-opnd (car opnds))
        ...
        (let* ((sn-loc
                  (if (and loc (not (eq? first-opnd loc))) (sn-opnd first-opnd sn-loc) sn))
          ...
          (let* ((offset
            ...
            (if (and loc (not (eq? first-opnd loc))) (copy-opnd-to-loc first-opnd loc sn))))))))
    ...
    (let ((sn-loc (if loc (sn-opnd loc sn) sn)))
      (let ((first-opnd (car opnds)) (second-opnd (cadr opnds)))
        (let* ((sn-loc
               ...
               (o1 (opnd->opnd68 first-opnd (temp-in-opnd68 o2) sn-loc)))
          (make-top-of-frame-if-stk-opnds68 o1 o2 sn-loc)
          (move-opnd68-to-loc68 (opnd68->true-opnd68 o2 (sn-opnd68 o1 sn-loc)) dtemp1)
          (emit-move.l (opnd68->true-opnd68 o1 sn-loc) atemp1)
          (if (eq? kind 'string)
              (begin
                (emit-asr.l (make-imm 3) dtemp1)
                (emit-move.b (make-imm 0)
...
(define (gen-compare* o1 o2 fs)
  ...
  (let ((order-1-2
           (cond
             ((imm? o1) (cmp-n-to-opnd68 (imm-val o1) (opnd68->true-opnd68 o2 fs)))
             ((imm? o2) (not (cmp-n-to-opnd68 (imm-val o2) (opnd68->true-opnd68 o1 fs))))
  ...
  (let ((opnd (car opnds)))
    (let ((o (opnd->opnd68 opnd #f fs)))
      (define (mask-test set-reg correction)
        (emit-btst (if (= correction 0)
                       (if (dreg? o)
                           o
                           (begin (emit-move.l (opnd68->true-opnd68 o fs) dtemp1) dtemp1))
                       (begin
