...
(for-each (lambda (x y)
            ...
            (for-each (lambda (f)
                        (set! i (+ 1 i))
                        (cond
                          ((and (= i j)) (cond ((not (f x)) (test #t f x))))
                          ((f x) (test #f f x)))
                        (cond
                          ((and (= i j)) (cond ((not (f y)) (test #t f y))))
                          ((f y) (test #f f y))))
                      disjoint-type-functions))
...
(test 5 'set! (+ x 1))
(SECTION 4 2 1)
(test 'greater 'cond (cond ((> 3 2) 'greater) ((< 3 2) 'less)))
(test 'equal 'cond (cond ((> 3 3) 'greater) ((< 3 3) 'less) (else 'equal)))
(test 2 'cond (cond ((assv 'b '((a 1) (b 2))) => cadr) (else #f)))
(test 'composite
      'case
...
(test '((6 1 3) (-5 -2))
      ...
      (let loop ((numbers '(3 -2 1 6 -5)) (nonneg '()) (neg '()))
        (cond
          ...
          (else (loop (cdr numbers) (cons (car numbers) nonneg) neg)))))
(SECTION 4 2 6)
...
;; r5rs doesn't let you redefine imported bindings.
;; (define old-+ +)
...
(test 99
      'internal-define
      (letrec ((foo (lambda (arg) (or arg (and (procedure? foo) (foo 99))))))
        (define bar (foo #f))
        (foo #f)))
(test 77
      'internal-define
      (letrec ((foo 77) (bar #f) (retfoo (lambda () foo)))
        (define baz (retfoo))
        (retfoo)))
(SECTION 6 1)
(test #f not #t)
...
(test #f eqv? #f 'nil)
(let ((p (lambda (x) x))) (test #t eqv? p p))
...
(test #t 'standard-case (string=? (symbol->string 'a) (symbol->string 'A)))
(test #t
      'standard-case
      (or (string=? (symbol->string 'a) "A") (string=? (symbol->string 'A) "a")))
(define (str-copy s)
...
(test -1 modulo -13 -4)
(test -1 remainder -13 -4)
(define (divtest n1 n2) (= n1 (+ (* n2 (quotient n1 n2)) (remainder n1 n2))))
...
(define (test-inexact)
  ...
  (let ((x (string->number "4195835.0")) (y (string->number "3145727.0")))
    (test #t 'pentium-fdiv-bug (> f1.0 (- x (* (/ x y) y)))))
  (report-errs))

(define (test-bignum)
  (define tb
    (lambda (n1 n2) (= n1 (+ (* n2 (quotient n1 n2)) (remainder n1 n2)))))
  (newline)
...
(define (next-leaf-generator obj eot)
  (letrec ((return #f)
           (cont
              (lambda (x) (recur obj) (set! cont (lambda (x) (return eot))) (cont #f)))
           (recur
              (lambda (obj)
                (if (pair? obj)
                    (for-each recur obj)
                    (call-with-current-continuation (lambda (c) (set! cont c) (return obj)))))))
    (lambda ()
      (call-with-current-continuation (lambda (ret) (set! return ret) (cont #f))))))
(define (leaf-eq? x y)
  (let* ((eot (list 'eot))
    ...
    (letrec ((loop
                (lambda (x y)
                  (cond ((not (eq? x y)) #f) ((eq? eot x) #t) (else (loop (xf) (yf)))))))
      (loop (xf) (yf)))))
(define (test-cont)
  (newline)
...
(define (test-delay)
  ...
  (test '(3 3) 'delay (let ((p (delay (+ 1 2)))) (list (force p) (force p))))
  (test 2
        'delay
        (letrec ((a-stream
                    (letrec ((next (lambda (n) (cons n (delay (next (+ n 1))))))) (next 0)))
                 (head car)
                 (tail (lambda (stream) (force (cdr stream)))))
          (head (tail (tail a-stream)))))
  (letrec ((count 0)
           (p (delay (begin (set! count (+ count 1)) (if (> count x) count (force p)))))
           (x 5))
    (test 6 force p)
    (set! x 10)
    (test 6 force p))
  (test 3
        'force
        (letrec ((p (delay (if c 3 (begin (set! c #t) (+ (force p) 1))))) (c #f))
          (force p)))
  (report-errs))
...
(define display-test-obj
  '(#t #f a () 9739 -3 . #((test) te " " st test #() b c)))
(define load-test-obj (list 'define 'foo (list 'quote write-test-obj)))
(test #t
      call-with-output-file
      "tmp1"
      (lambda (test-file)
