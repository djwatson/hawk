...
static void trace2(long* lst, bool incr);
static void trace2_cb(long* lst, void* ctx) {
  trace2(lst, false);
}
static void trace2(long* lst, bool incr) {
  for(uint64_t i = 0; i < arrlen(lst); i++) {
    auto item = lst[i];
    ...
    if (incr) {
      ...
      if(--((uint32_t*)ptr)[1] == 0) {
	// Recursive decrement
	trace_heap_object((void*)ptr, trace2_cb, NULL);
	//free((void*)ptr);
      }
    }
  }
