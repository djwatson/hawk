...
void emit_reg_reg(emit_state *s, uint8_t opcode, uint8_t src, uint8_t dst);
...
#define p (s->p)
void emit_rex(emit_state *s, uint8_t w, uint8_t r, uint8_t x, uint8_t b) {
  *(--p) = 0x40 | (w << 3) | (r << 2) | (x << 1) | b;
}

void emit_modrm(emit_state *s, uint8_t mod, uint8_t reg, uint8_t rm) {
  *(--p) = (mod << 6) | (reg << 3) | rm;
}

void emit_sib(emit_state *s, uint8_t scale, uint8_t index, uint8_t base) {
  *(--p) = (scale << 6) | ((0x7 & index) << 3) | ((0x7 & base));
}

void emit_imm8(emit_state *s, uint8_t imm) { *(--p) = imm; }

void emit_imm64(emit_state *s, int64_t imm) {
  p -= sizeof(int64_t);
  memcpy(p, &imm, sizeof(imm));
}

void emit_imm32(emit_state *s, int32_t imm) {
  p -= sizeof(int32_t);
  memcpy(p, &imm, sizeof(imm));
}
...
void emit_call_indirect(emit_state *s, uint8_t r) {
  emit_modrm(s, 0x3, 0x2, 0x7 & r);
  *(--p) = 0xff;
  emit_rex(s, 1, 0, 0, r >> 3);
}

void emit_call_indirect_mem(emit_state *s, int32_t offset) {
  emit_imm32(s, offset);
...
void emit_call32(emit_state *s, int32_t offset) {
  emit_imm32(s, offset);
  *(--p) = 0xe8;
}
...
void emit_cmp_mem32_imm32(emit_state *s, int32_t offset, uint8_t r1,
                          int32_t imm) {
  emit_imm32(s, imm);
  assert(r1 != RSP);
  assert(r1 != RBP);
...
void emit_cmp_reg_imm32(emit_state *s, uint8_t r, int32_t imm) {
...
void emit_cmp_reg_reg(emit_state *s, uint8_t src, uint8_t dst) {
...
void emit_jmp_indirect(emit_state *s, int32_t offset) {
  emit_imm32(s, offset);
  emit_modrm(s, 0x0, 4, RBP);
  *(--p) = 0xff;
}

void emit_jmp_abs(emit_state *s, enum registers r) {
...
void emit_reg_reg2(emit_state *s, uint8_t opcode, uint8_t src, uint8_t dst) {
  emit_modrm(s, 0x3, 0x7 & src, 0x7 & dst);
  *(--p) = opcode;
  *(--p) = 0x0f;
  emit_rex(s, 1, src >> 3, 0, dst >> 3);
...
void emit_mem_reg_sib(emit_state *s, uint8_t opcode, int32_t offset,
                      uint8_t scale, uint8_t index, uint8_t base, uint8_t reg) {
...
void emit_mem_reg_sib2(emit_state *s, uint8_t opcode, int32_t offset,
...
void emit_mem_reg(emit_state *s, uint8_t opcode, int32_t offset, uint8_t r1,
                  uint8_t r2) {
  if (low3bits(r1) == RSP) {
...
void emit_mem_reg2(emit_state *s, uint8_t opcode, int32_t offset, uint8_t r1,
                   uint8_t r2) {
  if (low3bits(r1) == RSP) {
...
void emit_op_imm32(emit_state *s, uint8_t opcode, uint8_t r1, uint8_t r2,
                   int32_t imm) {
  emit_imm32(s, imm);
  emit_reg_reg(s, opcode, r1, r2);
}

void emit_arith_imm(emit_state *s, enum ARITH_CODES op, uint8_t src,
                    int32_t imm) {
  if ((int32_t)((int8_t)imm) == imm) {
    *(--p) = imm;
...
void emit_cmovl(emit_state *s, uint8_t dst, uint8_t src) {
  emit_modrm(s, 0x3, 0x7 & src, 0x7 & dst);
  *(--p) = 0x4c;
  *(--p) = 0x0f;
  emit_rex(s, 1, src >> 3, 0, dst >> 3);
