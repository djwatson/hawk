...
`boot/img.scm.bc.zstd` when zstd is available.  This is the recommended path
for users who only want to build and run Hawk.
...
Git checkouts do not include the generated boot image.  Build with
`-DBOOTSTRAP=ON` so the build first creates a bootstrap-capable `hawk`, uses
Gauche to compile the Scheme sources, then writes `boot/img.scm.bc.zstd` when
zstd is available, or `boot/img.scm.bc` otherwise.
...
bootstrap build only to generate the boot image.
