...
#undef RECORD_BIN_CMP
  ...
  case OP_CLOSURE_GET: {
    // We need the closure type typechecked, because currently
    // calls to unknown objects call CLOSURE_GET to get the bcfunc*
    // If we fixed that, most closure_gets are generated by the compiler, and
    // don't need this typecheck.
    auto clo = stack_load(state, stack, instr.v1, true);
