...
(define (closure-conversion sexp)
  ...
  (define (substitute-free f bindings)
    (if (atom? f)
	(if (assq f bindings) `(closure-get ,(cdr (assq f bindings))) f)
	(case (car f)
	  ((quote) f)
	  (else (imap (lambda (f) (substitute-free f bindings)) f)))))
	...
	(case (car f)
	  ...
	  ((lambda)
	   (let* (
		  ...
		  (cl-name (gensym 'closure))
		  (free-bind (map cons free-vars (iota (length free-vars))))
		  (new-body (substitute-free body free-bind)))
	     (display (format "FOUND FREE: ~a\n" free-vars))
	     `(let ((,cl-name (closure (lambda ,(second f) ,@new-body) ,(length free-vars))))
		,@(map (lambda (v) `(closure-set! ,cl-name ,(car v) ,(cdr v))) free-bind)
		,cl-name)))
